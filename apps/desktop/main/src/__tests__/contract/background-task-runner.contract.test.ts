...
async function main(): Promise<void> {
  ...
  await runScenario(
    "BE-TG-S1 error: execute throws returns error status",
    async () => {
      const runner = createBackgroundTaskRunner();
      const result = await runner.run({
        execute: async () => {
  ...
  await runScenario(
    "BE-TG-S1 timeout: slow task returns timeout status",
    async () => {
      const runner = createBackgroundTaskRunner();
      const result = await runner.run({
        execute: (signal) =>
  ...
  await runScenario(
    "BE-TG-S1 aborted: external signal returns aborted status",
    async () => {
      const runner = createBackgroundTaskRunner();
  ...
  await runScenario(
    "BE-TG-S1 crashed: crashAll returns crashed status",
    async () => {
      const runner = createBackgroundTaskRunner();
      const resultPromise = runner.run({
  ...
  await runScenario(
    "BE-TG-S1 no ghost execution: aborted task does not resolve with value",
    async () => {
      const runner = createBackgroundTaskRunner();
      const controller = new AbortController();
