...
async function main(): Promise<void> {
  await runScenario("BE-TG-S3 timeout aborts work: signal is aborted after timeout", async () => {
    ...
    assert.equal(signalAbortedDuringWork, true, "signal must be aborted to stop work");
  });

  await runScenario("BE-TG-S3 no ghost execution: work does not produce value after timeout", async () => {
    ...
    assert.equal(result.status, "timeout");
    assert.equal(ghostValue, undefined, "ghost execution must not occur after timeout");
  });

  await runScenario("BE-TG-S3 external abort: caller can cancel before timeout", async () => {
  ...
  await runScenario("BE-TG-S3 crash propagates: utility process crash aborts all inflight", async () => {
    const handler = createIpcHandler(5000);
    ...
    assert.ok(results.every((r) => r.status === "crashed"), "all inflight must be crashed");
  });
