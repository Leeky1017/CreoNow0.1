...
/**
 * WB-FE-I18N-CORE-S1: DashboardPage must have no hardcoded visible strings.
 *
 ...
 */
describe("WB-FE-I18N-CORE-S1: DashboardPage i18n guard", () => {
  const filePath = path.resolve(__dirname, "DashboardPage.tsx");
  ...
  it("has no hardcoded Chinese characters in visible strings", () => {
    ...
    expect(
      matches,
      `Found ${matches?.length ?? 0} residual Chinese character(s) in DashboardPage.tsx`,
    ).toBeNull();
  });
