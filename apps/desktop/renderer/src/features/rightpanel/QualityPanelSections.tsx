...
import type { CheckGroup, PanelStatus } from "../quality-gates/QualityGatesPanel";

type JudgeModelState = { status: "ready" } | { status: "downloading" } | { status: "not_ready" } | { status: "error"; error: { code: string; message: string } };
type ConstraintsData = { items: string[] };

export type { JudgeModelState, ConstraintsData };

function formatJudgeState(state: JudgeModelState, t: TFunction): { label: string; status: "ready" | "downloading" | "error" | "not_ready" } {
  switch (state.status) {
    case "ready": return { label: t("rightPanel.quality.ready"), status: "ready" };
    case "downloading": return { label: t("rightPanel.quality.downloading"), status: "downloading" };
    case "not_ready": return { label: t("rightPanel.quality.notReady"), status: "not_ready" };
    case "error": return { label: t("rightPanel.quality.errorWithCode"), status: "error" };
  }
}

function StatusDot(props: { status: "ready" | "downloading" | "error" | "not_ready" | "loading" }): JSX.Element {
  const colors = {
  ...
  if (props.status === "downloading") {
    return <span className="inline-block w-2 h-2 rounded-full bg-[var(--color-info)] animate-pulse" />;
  }
  return <span className={`inline-block w-2 h-2 rounded-full ${colors[props.status]}`} />;
}
...
export function JudgeStatusSection(props: {
      ...
      <Card className="p-3 rounded-[var(--radius-md)]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <Text size="small" color="muted">{t("rightPanel.quality.loadingJudgeStatus")}</Text>
          </div>
        </div>
      </Card>
    );
      ...
      <Card className="p-3 rounded-[var(--radius-md)] border-[var(--color-error)]/20">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <Text data-testid="quality-panel-judge-error" size="small" color="muted">
              <span data-testid="quality-panel-judge-error-code">{getHumanErrorMessage(error)}</span>
            </Text>
          </div>
        </div>
      </Card>
  ...
  if (!state) {
    return (
      <Card className="p-3 rounded-[var(--radius-md)]">
        <Text size="small" color="muted">{t("rightPanel.quality.judgeStatusUnavailable")}</Text>
      </Card>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <Text size="small" color="default">
            ...
            <span data-testid="quality-panel-judge-status" data-status={status} className="font-medium">{label}</span>
          </Text>
        </div>
        {status !== "ready" && status !== "downloading" && (
          <Button data-testid="quality-panel-judge-ensure" variant="secondary" size="sm" onClick={onEnsure} disabled={ensureBusy} loading={ensureBusy}>
...
export function ConstraintsSection(props: {
  ...
  if (loading) {
    return (
      <Card className="p-3 rounded-[var(--radius-md)]">
        <Text size="small" color="muted">{t("rightPanel.quality.loadingConstraints")}</Text>
      </Card>
    );
  }
  ...
  if (error) {
    return (
      <Card className="p-3 rounded-[var(--radius-md)] border-[var(--color-error)]/20">
        <Text data-testid="quality-panel-constraints-error" size="small" color="muted">
          <span data-testid="quality-panel-constraints-error-code">{getHumanErrorMessage(error)}</span>
        </Text>
      </Card>
    );
  }
      ...
      {count > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {constraints?.items.slice(0, 5).map((item, idx) => (
            <span key={idx} className="inline-block px-2 py-0.5 text-[10px] rounded-full bg-[var(--color-bg-hover)] text-[var(--color-fg-muted)]">
...
export function buildCheckGroups(judgeState: JudgeModelState | null, constraints: ConstraintsData | null, t: TFunction): CheckGroup[] {
  const groups: CheckGroup[] = [];

  if (judgeState) {
    const judgeStatus = judgeState.status === "ready" ? "passed"
      : judgeState.status === "error" ? "error"
      : judgeState.status === "downloading" ? "running"
      : "warning";
    groups.push({
      id: "system",
