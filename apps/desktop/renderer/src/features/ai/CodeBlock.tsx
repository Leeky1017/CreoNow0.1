  ...
  return (
    <div className="my-3 border border-[var(--color-border-default)] rounded-[var(--radius-md)] overflow-hidden bg-[var(--color-bg-base)]">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-[var(--color-bg-raised)] border-b border-[var(--color-separator)]">
        <span className="text-(--text-status) text-[var(--color-fg-muted)] uppercase tracking-wide">
          {props.language || "code"}
        </span>
        <div className="flex items-center gap-1">
          <Button
            type="button"
            onClick={handleCopy}
            className="focus-ring px-2 py-0.5 text-(--text-status) text-[var(--color-fg-muted)] hover:text-[var(--color-fg-default)] hover:bg-[var(--color-bg-hover)] rounded transition-colors"
          >
          ...
          {props.onApply && (
            <Button
              ...
              className="focus-ring px-2 py-0.5 text-(--text-status) text-[var(--color-fg-accent)] hover:bg-[var(--color-bg-hover)] rounded transition-colors"
            >
      ...
      {/* Code content */}
      <pre className="m-0 p-3 overflow-x-auto text-(--text-caption) leading-[1.6] text-[var(--color-fg-default)] font-[var(--font-family-mono)]">
        {/* 审计：v1-13 #014 KEEP */}
