...
describe('V1-22 Brand Identity Guard', () => {
  const tokensPath = path.resolve(__dirname, '../../../../../design/system/01-tokens.css');
  ...
  it('should have illustrations directory with SVGs', () => {
    ...
    const svgs = fs.readdirSync(illustrationsDir).filter(f => f.endsWith('.svg'));
    expect(svgs.length).toBeGreaterThanOrEqual(6);
  });

  it('EmptyState should support illustration prop', () => {
    const emptyStatePath = path.resolve(__dirname, '../components/composites/EmptyState.tsx');
