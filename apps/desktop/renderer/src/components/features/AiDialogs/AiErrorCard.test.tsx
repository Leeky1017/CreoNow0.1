...
describe("渲染", () => {
  ...
  it("用量限制错误显示 Upgrade Plan 按钮", () => {
    render(<AiErrorCard error={usageLimitError} onUpgradePlan={vi.fn()} />);
    expect(
  ...
  it("服务错误显示 Check Status 按钮", () => {
    render(<AiErrorCard error={serviceError} onCheckStatus={vi.fn()} />);
    expect(
  ...
  it("用量限制错误提供 onViewUsage 时显示 View Usage 按钮", () => {
    render(<AiErrorCard error={usageLimitError} onViewUsage={vi.fn()} />);
    expect(
  ...
  it("无 errorCode 时不显示错误码区域", () => {
    render(<AiErrorCard error={serviceError} errorCodeTestId="error-code" />);
...
describe("交互", () => {
  it("点击 Retry 按钮触发 onRetry 回调", async () => {
    ...
    render(<AiErrorCard error={connectionError} onRetry={onRetry} />);
  ...
  it("点击 Upgrade Plan 触发 onUpgradePlan 回调", async () => {
    ...
    const user = userEvent.setup();
    render(
      <AiErrorCard error={usageLimitError} onUpgradePlan={onUpgradePlan} />,
    );
  ...
  it("点击 Check Status 触发 onCheckStatus 回调", async () => {
    ...
    const user = userEvent.setup();
    render(<AiErrorCard error={serviceError} onCheckStatus={onCheckStatus} />);
  ...
  it("点击 View Usage 触发 onViewUsage 回调", async () => {
    ...
    const user = userEvent.setup();
    render(<AiErrorCard error={usageLimitError} onViewUsage={onViewUsage} />);
...
describe("状态 — 倒计时", () => {
  ...
  it("倒计时期间禁用 Retry 按钮", () => {
    render(<AiErrorCard error={rateLimitError} onRetry={vi.fn()} />);
