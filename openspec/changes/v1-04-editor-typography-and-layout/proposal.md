...
| Featured image        | 有（gradient overlay）       | `EditorFeaturedImage.tsx` 组件已实现                                          | ✅   |
...
- **上游依赖**: v1-01（Design Token 补完）已合并 ✅ — `--text-display-*`、`--leading-relaxed`、`--tracking-tight` 等 typography token 已就位
