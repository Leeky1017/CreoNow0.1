...
- 当前活跃 change 数量：**25**（24 止血 + 1 个 T-MIG umbrella change；Wave 0 门禁 6 个与 Wave 0.5 审计补丁 6 个均已完成）
...
### Wave 0.5: 审计补丁 — 制度门禁与视觉回归（G0.5-01 ~ G0.5-06，已完成）

| ID      | slug                                | GitHub Issue              | 任务簇    | 载体类型        | 来源                               |
| ------- | ----------------------------------- | ------------------------- | --------- | --------------- | ---------------------------------- |
| G0.5-01 | `g05-01-require-describe-in-tests`  | #1072（PR #1078，已完成） | W0.5-GATE | OpenSpec Change | GAP-8: 测试结构规范无自动化拦截    |
| G0.5-02 | `g05-02-backend-coverage-threshold` | #1073（PR #1079，已完成） | W0.5-GATE | OpenSpec Change | GAP-2: Backend coverage 阈值未设置 |
| G0.5-03 | `g05-03-format-check-ci`            | #1074（PR #1080，已完成） | W0.5-GATE | OpenSpec Change | GAP-3: `format:check` 未接入 CI    |
| G0.5-04 | `g05-04-review-audit-script`        | #1075（PR #1081，已完成） | W0.5-GATE | OpenSpec Change | GAP-6: 审计一键脚本未落地          |
| G0.5-05 | `g05-05-e2e-path-mapping`           | #1076（PR #1082，已完成） | W0.5-GATE | OpenSpec Change | GAP-7: E2E 关键路径映射不透明      |
| G0.5-06 | `g05-06-visual-regression-testing`  | #1077（PR #1083，已完成） | W0.5-GATE | OpenSpec Change | 视觉回归测试基础设施为零           |
...
| #1072 | G0.5-01     | 已完成（PR #1078） |
| #1073 | G0.5-02     | 已完成（PR #1079） |
| #1074 | G0.5-03     | 已完成（PR #1080） |
| #1075 | G0.5-04     | 已完成（PR #1081） |
| #1076 | G0.5-05     | 已完成（PR #1082） |
| #1077 | G0.5-06     | 已完成（PR #1083） |
| #1035 | A0-08       | 活跃               |
| #986  | A0-01       | 活跃               |
| #992  | A0-02       | 活跃               |
| #993  | A0-03       | 活跃               |
| #1002 | A0-04       | 已完成（PR #1048） |
| #987  | A0-05       | 活跃               |
| #999  | A0-06       | 已完成（PR #1060） |
| #1000 | A0-07       | 已完成（PR #1061） |
| #990  | A0-09       | 已完成（PR #1053） |
| #1003 | A0-10       | 活跃               |
| #1001 | A0-11       | 已完成（PR #1062） |
| #1004 | A0-12       | 活跃               |
| #981  | A0-13       | 已完成（PR #1044） |
| #994  | A0-14       | 活跃               |
| #995  | A0-15       | 活跃               |
| #991  | A0-16       | 活跃               |
| #996  | A0-17       | 活跃               |
| #997  | A0-18       | 已完成（PR #1046） |
| #998  | A0-19       | 已完成（PR #1063） |
| #983  | A0-20       | 已完成（PR #1043） |
| #988  | A0-21       | 活跃               |
| #989  | A0-22       | 活跃               |
| #984  | A0-23       | 已完成（PR #1065） |
| #985  | A0-24       | 已完成（PR #1042） |

### Wave 0.5 Issue（6 个；均已完成并合并到 `main`）

| 对应 Change | 任务                               | 状态                      |
| ----------- | ---------------------------------- | ------------------------- |
| G0.5-01     | ESLint `require-describe-in-tests` | #1072 / PR #1078 / 已完成 |
| G0.5-02     | 后端 coverage threshold            | #1073 / PR #1079 / 已完成 |
| G0.5-03     | `format:check` 接入 CI             | #1074 / PR #1080 / 已完成 |
| G0.5-04     | 审计一键脚本 `review-audit.sh`     | #1075 / PR #1081 / 已完成 |
| G0.5-05     | E2E 关键路径映射表                 | #1076 / PR #1082 / 已完成 |
| G0.5-06     | 前端视觉回归测试基础设施           | #1077 / PR #1083 / 已完成 |
...
| ID      | Issue                     | 任务                               | 说明                                                        | 来源     |
| ------- | ------------------------- | ---------------------------------- | ----------------------------------------------------------- | -------- |
| G0.5-01 | #1072（PR #1078，已完成） | ESLint `require-describe-in-tests` | 测试结构规范自动拦截，存量目录临时 `warn` 豁免              | GAP-8    |
| G0.5-02 | #1073（PR #1079，已完成） | 后端 coverage threshold            | `vitest.config.core.ts` 添加 `thresholds`，CI 阻断          | GAP-2    |
| G0.5-03 | #1074（PR #1080，已完成） | `format:check` 接入 CI             | `ci.yml` 独立 `format-check` job + `ci` meta-gate `needs`   | GAP-3    |
| G0.5-04 | #1075（PR #1081，已完成） | 审计一键脚本 `review-audit.sh`     | 封装 AGENTS.md §6.4 全部命令                                | GAP-6    |
| G0.5-05 | #1076（PR #1082，已完成） | E2E 关键路径映射表                 | `05-e2e-testing-patterns.md` 新增映射章节 + 空洞评估        | GAP-7    |
| G0.5-06 | #1077（PR #1083，已完成） | 前端视觉回归测试基础设施           | Playwright + Storybook 截图对比，Dark/Light 双主题，CI 集成 | 审计发现 |

**Wave 0.5 完成标志**：

1. 已完成：新 PR 中无 `describe()` 的测试文件被 ESLint 自动拦截
2. 已完成：后端 coverage 下降时 CI 失败
3. 已完成：格式不一致时 CI 失败
4. 已完成：审计 Agent 可一键执行全部必跑命令
5. 已完成：7 条关键路径 ↔ E2E 映射表完整
6. 已完成：原语 + 布局 + 核心功能组件有 dark/light 双主题视觉 baseline 截图，样式变更时 CI 失败
...
### Wave 1（前置条件已满足；后续任务必须从最新 `main` 基线重开）

> 2026-03-12 起，Wave 0.5 已全部并入 `main`。所有后续 Wave 1+ 工程任务一律基于最新 `main` 新建分支 / worktree 执行，不复用旧基线上的历史 PR 或陈旧 worktree。
