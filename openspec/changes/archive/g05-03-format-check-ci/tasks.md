...
- [x] 确认 `pnpm format:check` 命令可本地执行且通过
- [x] 确认 `ci.yml` 中无此命令
...
- [x] 在 `.github/workflows/ci.yml` 的 `lint-and-typecheck` job 中新增 step：
  ```yaml
  - name: Format check
    run: pnpm format:check
  ```
- [x] 如果选择独立 job 方案，需同步更新 `ci` meta-job 的 `needs`

### Task 2.2: 文档同步

**映射验收标准**: AC-4

- [x] 更新 `07-test-command-and-ci-map.md` 格式化检查状态
