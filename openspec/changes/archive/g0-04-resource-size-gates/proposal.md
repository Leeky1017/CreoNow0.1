...
- 运行 `pnpm -C apps/desktop build` 后分析 `apps/desktop/dist/` 目录下的 `.js` / `.mjs` / `.cjs` 文件大小
- 记录 baseline（初始各 chunk 大小）
- 若总大小超过 baseline + 容许增长（当前阈值 10%），则直接阻断
- PR / CI 中输出 bundle 大小变化报告，并在构建产物缺失时显式失败
...
4. **不单独扩展到运行时性能画像**——本 Change 只约束构建产物体积与缺产物失败，不覆盖运行时 FPS / 内存指标
...
| — | Bundle 大小无预算（AMP 补充识别，未编入 28-Pattern 正式序号） | Guard `bundle-size-budget` | required gate（构建产物缺失即失败） |
