...
- [x] 每行填写 Phase 1 核查的具体证据
- [x] 标记每行的差距级别：✅ 已闭环 / ⚠️ 部分实现 / ❌ 完全缺失
...
- [x] 按 Delta Spec 定义的结构，编写三个方案的五维度对比
- [x] 方案 A（隐藏入口）：评估实现成本（隐藏哪些组件、移除哪些 i18n key）
- [x] 方案 B（最小闭环）：评估需要新增的模块数量和预估工作量等级（S/M/L/XL）
- [x] 方案 C（标注 Coming Soon）：评估与 A0-15 占位 UI 策略的一致性

### Task 3.2: 决策建议

**映射验收标准**: AC-5

- [x] 基于事实核查结论和方案对比，给出推荐方案
- [x] 推荐理由必须引用具体证据（Q 编号 + 核查结论），不做空口建议
- [x] 明确标注：最终决策权在 Owner
...
- [x] 自检 Q1-Q10 是否全部有明确结论和证据
- [x] 自检三方案对比是否五维度完整
- [x] 自检与 A0-15 策略一致性是否已确认
- [x] 自检 A0-17 可消费性——模拟 A0-17 执行 Agent 视角，确认凭本文档可直接行动
...
| 验收标准                | 对应任务           | 验证方式           | 状态 |
| ----------------------- | ------------------ | ------------------ | ---- |
| AC-1: Q1-Q10 全部已回答 | Task 1.1, 1.2, 1.3 | 文档审查           | [x]  |
| AC-2: 无未闭项          | Task 1.1, 1.2, 1.3 | 搜索"待确认"/"TBD" | [x]  |
| AC-3: 三方案列出        | Task 3.1           | 文档结构审查       | [x]  |
| AC-4: 五维度完整        | Task 3.1           | 表格完整性         | [x]  |
| AC-5: 推荐方案有据      | Task 3.2           | 引用 Q 编号验证    | [x]  |
| AC-6: 决策门槛四项满足  | Task 3.3           | 自检清单           | [x]  |
| AC-7: A0-17 可消费      | Task 3.3           | A0-17 视角模拟     | [x]  |

---

## Done 定义

- [x] Q1-Q10 全部有代码证据支撑的明确回答
- [x] 能力差距矩阵完成，无空白行
- [x] 三方案五维度对比完成
- [x] 推荐方案引用具体证据
- [x] 决策门槛四项自检通过
- [x] PR body 包含 `Closes #<N>`（Issue 创建后补充）
