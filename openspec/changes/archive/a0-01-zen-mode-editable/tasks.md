...
- [x] 测试：禅模式 `open=true` 时，渲染 `EditorContent` 组件（不渲染静态 `<p>` 元素）
- [x] 测试：渲染区域不包含 `data-testid="zen-cursor"`（BlinkingCursor 已移除）
- [x] 测试：通过 mock editor 实例模拟用户输入，断言 `editor.getHTML()` 包含新内容
- [x] 测试：禅模式 `open=false` 时不渲染任何内容（保持现有行为）
...
- [x] 测试：禅模式 `open` 从 `true` 变为 `false` 后，editor 实例中的内容不变（无内容丢失）
- [x] 测试：退出禅模式后，editor 实例的 undo 历史包含禅模式中的操作（可 `Cmd+Z` 撤销）
- [x] 测试：不存在第二个 editor 实例（`createEditor` 仅被调用一次）
...
- [x] 测试：`layoutStore.zenMode === true` 时，BubbleMenu 的 `shouldShow` 返回 `false`
- [x] 测试：`layoutStore.zenMode === true` 时，slash command extension 不响应 `/` 输入
- [x] 测试：`layoutStore.zenMode === true` 时，`EditorToolbar` 不在 DOM 中
- [x] 测试：禅模式下，`Cmd/Ctrl+B` 仍然切换选中文字的加粗状态（快捷键通过 TipTap 处理，不受 UI 隐藏影响）
...
- [x] 测试：禅模式下编辑内容后，`useAutosave` 的 debounce 触发 save（mock IPC，断言 `file:document:save` 被调用）
- [x] 测试：save 成功后，传入 `ZenModeStatus` 的 `saveStatus` 值更新为已保存状态对应文案
...
- [x] 测试：editor 内容为空时进入禅模式，标题区域文本为 `t("zenMode.untitledDocument")`
- [x] 测试：editor 内容为空时，编辑区域显示 placeholder `t("zenMode.startWriting")`
- [x] 测试：输入第一个字符后 placeholder 不再显示
...
- [x] 测试：禅模式覆盖层具有 `role="dialog"` 和 `aria-label` 属性
- [x] 测试：`aria-label` 值为 `t("zenMode.a11y.dialogLabel")`
- [x] 测试：进入禅模式后，`document.activeElement` 在 EditorContent 容器内
...
- [x] 测试：`zh-CN.json` 包含 `zenMode.untitledDocument`、`zenMode.startWriting`、`zenMode.a11y.dialogLabel`
...
- [x] 修改 `ZenModeProps`：移除 `content: ZenModeContent`，新增 `editor: Editor`（TipTap 实例）和 `title: string`
- [x] 删除 `BlinkingCursor` 组件、`ZenModeContent` 接口、`showCursor` 相关逻辑
- [x] 在正文区域渲染 `<EditorContent editor={editor} />`，替换 `content.paragraphs.map()`
- [x] 为 `EditorContent` 外层容器添加 CSS class `zen-editor`，通过该 class 设置禅模式专属样式
- [x] 进入禅模式时（`open` 变为 `true`），调用 `editor.commands.focus('end')` 将焦点和光标移入编辑区
...
- [x] 创建 `.zen-editor .ProseMirror` 样式块（在 `ZenMode.tsx` 内联或独立 CSS 文件），设置 `font-family`、`font-size`、`line-height`、`color`、`caret-color` 等
- [x] 设置段落间距 `p + p { margin-top: 2rem; }`（对应原 `space-y-8`）
- [x] 设置选区颜色 `::selection { background: var(--color-accent); opacity: 0.3; }`
- [x] 禅模式下 `.ProseMirror` 无边框、无 outline（保持沉浸感），focus 时不显示默认 focus ring
- [x] 空编辑器 placeholder 样式：`.ProseMirror p.is-editor-empty:first-child::before` 设置 `content: attr(data-placeholder)`、`color: var(--color-fg-placeholder)`
...
- [x] 从 `editorStore` 获取 `editor` 实例
- [x] 从 `editor.getJSON()` 或 `documentContentJson` 中提取标题（复用 `extractZenModeContent` 的标题提取逻辑）
- [x] 字数统计和阅读时间保持现有逻辑
- [x] 传递 `editor` 和 `title` 给 `ZenMode`，移除 `content` prop 的构造
...
- [x] 导入 `useLayoutStore`，读取 `zenMode` 状态
- [x] 当 `zenMode === true` 时，`shouldShow` 返回 `false`
...
- [x] 当 `layoutStore.zenMode === true` 时，不激活 slash command 菜单

**文件**: `renderer/src/features/editor/slashCommands.ts` 或相关文件（修改）

### Task 2.6: 禅模式覆盖层无障碍属性

- [x] 为禅模式最外层 `<div>` 添加 `role="dialog"` 和 `aria-label={t("zenMode.a11y.dialogLabel")}`
- [x] 确认 `EditorContent` 生成的 `div.ProseMirror` 具有 `role="textbox"` 和 `aria-multiline="true"`（TipTap 默认行为，仅需验证）

**文件**: `renderer/src/features/zen-mode/ZenMode.tsx`（修改）

### Task 2.7: 新增 i18n key

- [x] 在 `zh-CN.json` 中新增 `zenMode.untitledDocument`（"无标题文档"）、`zenMode.startWriting`（"在此处开始创作…"）、`zenMode.a11y.dialogLabel`（"禅模式编辑器"）
- [x] 在 `en.json` 中新增相同 key 的英文翻译
- [x] 确认 key 命名符合现有 `zenMode.*` 命名空间
...
- [x] `extractZenModeContent` 不再需要返回 `paragraphs: string[]`，简化为仅返回 `{ title, wordCount }`
- [x] 更新所有调用方
...
- [x] 更新 `ZenMode.stories.tsx` 以适配新的 props（需提供 mock TipTap editor 实例）
- [x] 新增 story：编辑状态的禅模式、空文档的禅模式
...
- [x] 删除 `ZenModeContent` interface export（如仍有外部引用需一同清理）
- [x] 删除 `BlinkingCursor` 相关的测试断言和 story
- [x] 确认 CI 中 `vitest` 全量通过
