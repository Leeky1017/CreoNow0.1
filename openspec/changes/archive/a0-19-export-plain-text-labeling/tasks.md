...
- [x] 测试：Markdown / PDF / DOCX / TXT 的说明文案与真实能力一致
- [x] 测试：PDF / DOCX 不再显示“纯文本导出 · 不含格式”
- [x] 测试：中英文 locale 下文案一致映射
...
- [x] 为 ExportDialog 增加正常态与失败态 Story
- [x] 交互测试覆盖格式切换、提交失败、错误提示展示
...
- [x] 更新 `getFormatOptions()`，让 PDF / DOCX / Markdown / TXT 的说明文案与真实能力一致
- [x] 删除或替换过期的 plain-text-only 主文案
- [x] 保持所有用户可见文案走 i18n

### Task 2.2: 收口导出失败表面

- [x] 当导出前命中不支持结构时，在 ExportDialog 或关联错误表面显示明确原因
- [x] 确保错误态对屏幕阅读器可读

### Task 2.3: 同步 i18n

- [x] 更新 `zh-CN.json` / `en.json` 的导出文案 key
- [x] 删除或替换过期纯文本兜底文案在主界面的使用点
...
- [x] `pnpm -C apps/desktop storybook:build` 通过
- [x] Story 中可见真实能力说明与失败提示

### Task 3.2: 与 A0-04 对照复核

- [x] 对照 A0-04 的实现与 fixture 结果，确认 UI 未夸大也未缩小能力
...
| 条目                | 检查项                             | 状态 |
| ------------------- | ---------------------------------- | ---- |
| AC-1 UI 一致        | 四种格式说明与真实能力一致         | [x]  |
| AC-2 旧口径移除     | PDF / DOCX 不再显示纯文本旧提示    | [x]  |
| AC-3 失败明确       | 不支持结构时给出明确、可本地化原因 | [x]  |
| AC-4 Storybook 覆盖 | 正常态与失败态均有 Story           | [x]  |
| AC-5 i18n 收口      | 中英文 key 与主界面文案一致        | [x]  |
