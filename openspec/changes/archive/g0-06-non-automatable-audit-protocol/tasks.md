...
- [x] 在 `.github/pull_request_template.md` 末尾增加"非自动化检查"章节
- [x] 包含 4 个 checkbox：字体渲染、品牌调性、无障碍、CJK 场景
- [x] 标注"涉及 UI 改动时必填，后端独占 PR 可标 N/A"
- [x] checkbox 措辞必须可执行（不要"确认 XX"，要"验证 XX 并附截图/命令输出"）
- [x] 增加简短说明：什么情况下标 N/A 是合理的（仅改 `main/src/` 或 `scripts/` 或 `openspec/`）

### Task 1.2: 更新审计协议

**映射验收标准**: AC-2

- [x] 在 `docs/delivery-skill.md` §8 PRE-AUDIT 步骤中增加：
  - "检查是否包含至少 1 条产品行为验证"
  - "前端 PR 必须有用户场景验证（截图或命令输出）"
  - 明确"纯 diff review 不计入产品行为验证"
- [x] 在 FINAL-VERDICT 步骤中增加：
  - "必须回答：作为用户，修改后行为是否符合 spec.md 定义？"
  - "列举至少 1 个用户场景的预期行为与实际行为对照"
- [x] 同步更新 AGENTS.md §六相关条款（§6.1 #10 已声明 Tier 3 检查，此处补充具体执行细节）

### Task 1.3: 创建字体验收清单

**映射验收标准**: AC-3

- [x] 创建 `docs/references/font-verification-checklist.md`
- [x] 必须包含以下章节（每节均需具体可执行步骤，不能只写标题）：
...
- [x] 创建 `openspec/specs/ai-service/rate-limiting-rfc.md`
- [x] 必须包含以下章节：
...
- [x] 确认 AGENTS.md §六 与 delivery-skill.md §8 的审计条款一致
- [x] 确认 PR template 的检查项与 delivery-skill.md 的验收要求一致
- [x] 确认 font checklist 与 `design/system/01-tokens.css` 中的字体 token 一致
