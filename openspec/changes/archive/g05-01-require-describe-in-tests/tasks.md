...
- [x] 测试：无 `describe()` 的 `.test.ts` 文件触发 `missingDescribe` 错误
- [x] 测试：有 `describe()` 的 `.test.ts` 文件不触发
- [x] 测试：非测试文件（`.ts` / `.tsx`）不触发
- [x] 测试：有 `describe()` 但嵌套在条件语句中仍算有效
...
- [x] 在 `scripts/eslint-rules/` 下新增 `require-describe-in-tests.cjs`
- [x] 在 `scripts/eslint-rules/index.cjs` 中导出
- [x] 在 `.eslintrc.cjs` 中注册为 `"error"`

### Task 2.2: 存量豁免 overrides

**映射验收标准**: AC-5

- [x] 在 `.eslintrc.cjs` 的 `overrides` 中对存量目录设为 `"warn"`：
...
- [x] `01-philosophy-and-anti-patterns.md`：注明由 ESLint 自动强制
- [x] `06-guard-and-lint-policy.md`：新增此规则作为示例
- [x] `07-test-command-and-ci-map.md`：在 lint job 行中注明
