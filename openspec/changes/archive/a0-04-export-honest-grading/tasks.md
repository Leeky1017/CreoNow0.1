...
- [x] 新建固定导出 fixture，覆盖标题、粗体、斜体、列表、链接、图片
- [x] 测试：Markdown 导出结果保留 heading、emphasis、list、link、image 语义
- [x] 测试：Markdown 导出明确来自 TipTap JSON 结构，而非 `contentText` 拼接回退
...
- [x] 测试：PDF 导出对 fixture 中的结构节点生成可断言的样式块或渲染指令
- [x] 测试：PDF 导出不再直接以 `contentText` 整段写正文
- [x] 测试：图片节点进入 PDF 结构化转换层
...
- [x] 测试：DOCX 包内部存在 heading、run formatting、list 或 image relationship 对应结构
- [x] 测试：重复导出保持稳定输出路径与稳定结构
...
- [x] 测试：fixture 含暂不支持结构时，导出在写文件前失败
- [x] 测试：错误消息包含不支持的节点或 mark 类型
- [x] 测试：失败时不留下导出文件
...
- [x] 为导出服务增加结构化解析层，将 TipTap JSON 转为 Markdown / PDF / DOCX 共用的中间表示
- [x] 覆盖标题、段落、换行、粗体、斜体、下划线、有序/无序列表、引用块、链接、行内代码、分隔线、图片
- [x] 为不支持结构提供显式失败分支
...
- [x] Markdown：补齐缺失结构映射
- [x] PDF：从结构模型生成样式块与绘制顺序，不再从 `contentText` 直出
- [x] DOCX：按结构模型生成 heading、runs、lists、links、images，不再按纯文本拆段
- [x] TXT：继续保留纯文本导出，不受影响

### Task 2.3: 同步主 Spec

- [x] 修改 `openspec/specs/document-management/spec.md` 中「文档导出」Requirement，使 PDF / DOCX / Markdown 的真实能力与本变更一致
...
- [x] 为真实导出能力补充定向集成测试或 E2E，验证从编辑器到落盘的完整链路
- [x] 至少覆盖一条包含标题、列表、链接、图片的真实导出路径

### Task 3.2: 验证与文档回写准备

- [x] 运行导出相关 renderer / core 测试
- [x] 运行 `pnpm -C apps/desktop storybook:build`
- [x] 输出供 A0-06 / A0-07 / A0-11 回写使用的真实能力证据
