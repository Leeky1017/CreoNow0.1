...
- [x] 测试：调用 `installGlobalErrorHandlers({ onError })` 后，`window` 上存在 `unhandledrejection` 事件监听器
- [x] 测试：dispatch 一个 `PromiseRejectionEvent`（reason 为 `new Error('IPC timeout')`），`onError` 被调用，接收的 `GlobalErrorEntry.source` 为 `'unhandledrejection'`，`name` 为 `'Error'`，`message` 为 `'IPC timeout'`
- [x] 测试：dispatch 一个 `PromiseRejectionEvent`（reason 为非 Error 值，如字符串 `'raw rejection'`），`onError` 被调用，`name` 为 `'UnknownError'`，`message` 为 `'raw rejection'`
- [x] 测试：调用返回的卸载函数后，再 dispatch `PromiseRejectionEvent`，`onError` 不被调用
...
- [x] 测试：dispatch 一个 `ErrorEvent`（error 为 `new TypeError('Cannot read properties of undefined')`），`onError` 被调用，`source` 为 `'error'`，`name` 为 `'TypeError'`
- [x] 测试：dispatch 一个 `ErrorEvent`（error 为 `undefined`，仅有 `message`），`onError` 被调用，`name` 为 `'UnknownError'`
- [x] 测试：调用卸载函数后，再 dispatch `ErrorEvent`，`onError` 不被调用
...
- [x] 测试：同一 `name + message` 组合在 1000ms 内触发 3 次，`showToast` 仅被调用 1 次
- [x] 测试：同一组合第 1 次触发后等待 1001ms（`vi.advanceTimersByTime`），第 2 次触发，`showToast` 被调用第 2 次（冷却已过）
- [x] 测试：不同 `name + message` 组合在 200ms 内各触发 1 次，`showToast` 被调用 2 次
- [x] 测试：去重仅影响 Toast——每次触发时日志回调均被调用（3 次触发 → 日志回调 3 次）
...
- [x] 测试：mock `invoke('log:renderer-error', ...)` 抛出异常，全局兜底 handler 不触发新的 Toast
- [x] 测试：mock `invoke` 抛出异常后，`console.error` 被调用一次（静默记录）
- [x] 测试：日志 IPC 失败不产生新的 `unhandledrejection` 事件（不递归）
...
- [x] 测试：`installGlobalErrorHandlers` 在 `ReactDOM.createRoot` 之前被调用（通过 mock 两个函数，断言调用顺序）
...
- [x] 测试：`zh-CN.json` 和 `en.json` 均包含 `globalError.toast.title` 和 `globalError.toast.description`
- [x] 测试：中英文 key 值均为非空字符串
...
- [x] 在 `renderer/src/lib/globalErrorHandlers.ts` 中导出 `GlobalErrorEntry` interface
- [x] 包含字段：`source`（`'unhandledrejection' | 'error'`）、`name`（string）、`message`（string）、`stack`（string | undefined）、`timestamp`（string，ISO 8601）
...
- [x] 注册 `window.addEventListener('unhandledrejection', handler)`
...
- [x] 注册 `window.addEventListener('error', handler)`
...
- [x] 维护一个 `Map<string, number>` 记录去重键（`name + ':' + message`）的上次 Toast 触发时间戳
- [x] 新错误到达时，检查去重键的上次触发时间是否在 1000ms 以内；若是则跳过 Toast
- [x] 超过冷却期或新键则触发 Toast 并更新时间戳
- [x] 去重逻辑封装在 `globalErrorHandlers.ts` 内部或导出为可测试的辅助函数
...
- [x] 在 `onError` 回调中调用 `invoke('log:renderer-error', entry)`
- [x] IPC 调用使用 `void invoke(...).catch((err) => console.error(...))` 模式——发送失败仅 console.error，不抛出
- [x] 确保 `invoke` 的 `.catch()` handler 内不触发任何可能产生新 `unhandledrejection` 的操作
...
- [x] 在 `ReactDOM.createRoot(rootEl).render(...)` 之前调用 `installGlobalErrorHandlers({ onError })`
- [x] `onError` 回调中执行两件事：(1) 调用 `showToast`（含去重）；(2) 调用日志 IPC
- [x] Toast 调用需要导入并使用 `useToast` 暴露的命令式 API（或直接调用 Toast store）
...
- [x] 注册 IPC handler `log:renderer-error`，接收 `GlobalErrorEntry` 对象
- [x] 将条目序列化为 JSON 单行，追加写入 `userData/logs/renderer-errors.log`
- [x] 日志文件大小超过 5MB 时执行轮转（保留后半部分，删除前半部分）
- [x] 写入失败时 `console.error` 静默记录，不向 renderer 返回错误

**文件**: `main/src/services/logService.ts`（新建或扩展已有日志服务）

### Task 2.7: 新增 i18n key

- [x] 在 `zh-CN.json` 中新增 `globalError.toast.title` 和 `globalError.toast.description`
...
- [x] 在 preload 的 IPC 通道白名单中注册 `log:renderer-error`
- [x] 确保 renderer 可通过 `invoke` 正常调用该通道
...
- [x] 若 `main.tsx` 中的 `onError` 回调超过 20 行，提取为独立函数 `handleGlobalError` 放入 `globalErrorHandlers.ts`
- [x] 确保提取后 Toast 调用和日志 IPC 调用的职责清晰分离
