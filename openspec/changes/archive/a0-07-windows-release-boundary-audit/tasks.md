...
- [x] 创建 `docs/release/v0.1-windows-boundary.md`
- [x] 写入四个章节标题：核查概述、维度核查详情、就绪度总表、结论与建议
- [x] 在核查概述中填入核查目标、核查日期、核查方法说明

### Task 2.2: 编写维度核查详情

**映射验收标准**: AC-2, AC-3, AC-5

- [x] **代码签名**维度：填写当前状态、证据（Phase 1 收集的命令输出摘要）、就绪度标记、v0.1 处置建议
- [x] **自动更新**维度：填写当前状态、证据、就绪度标记、v0.1 处置建议
- [x] **备份能力**维度：引用 A0-08 结论或 `06` §4.1 审计结论，标注就绪度、处置建议
- [x] **崩溃可观测性**维度：填写当前状态、证据、就绪度标记、v0.1 处置建议
- [x] 每个维度的证据必须包含代码路径或搜索命令输出

### Task 2.3: 编写就绪度总表

**映射验收标准**: AC-4, AC-7

- [x] 制作一览表，四行四列：维度、就绪度标记、关键证据、v0.1 处置
- [x] 确认标记与 Phase 2.2 详情一致
- [x] 确认总表格式可被 A0-06 事实表直接引用

### Task 2.4: 编写结论与建议

- [x] 总结哪些维度可在 v0.1 对外承诺
- [x] 总结哪些维度需在发布文档中诚实声明"暂不承诺"
- [x] 标注方向性建议（如 v0.2 补 auto-update），但不承诺具体计划
...
- [x] 逐维度核对：就绪度标记是否与证据一致（搜索无结果却标 ✅ 为违规）
- [x] 确认无"据观察"/"据推测"式空口结论
- [x] 确认备份维度证据来源标注清晰（A0-08 结论 or `06` §4.1）
- [x] 确认文档未修改任何代码或配置文件
...
- [x] 模拟 A0-06 执行 Agent 视角：凭本文档可直接填写事实表「平台支持边界」章节
- [x] 确认就绪度总表无歧义、处置建议可操作
...
| 验收标准                 | 对应任务          | 验证方式              | 状态 |
| ------------------------ | ----------------- | --------------------- | ---- |
| AC-1: 文档存在且含四章节 | Task 2.1          | 文件存在 + 结构审查   | [x]  |
| AC-2: 四维度覆盖         | Task 1.1-1.4, 2.2 | 文档审查              | [x]  |
| AC-3: 每维度含四子项     | Task 2.2          | 文档审查              | [x]  |
| AC-4: 标记与代码一致     | Task 3.1          | 交叉验证              | [x]  |
| AC-5: 证据无空口         | Task 3.1          | 搜索"据观察"/"据推测" | [x]  |
| AC-6: 备份引用 A0-08     | Task 1.3, 3.1     | 引用链审查            | [x]  |
