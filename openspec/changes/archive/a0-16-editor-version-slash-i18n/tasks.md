...
- [x] 测试：读取 `zh-CN.json`，断言包含 Delta Spec 中定义的全部新增 key（≥ 28 个：3 编辑器 + 12 slash + 15 版本历史 - 2 AI 相同 key）
...
- [x] `EditorPane.tsx:400`：将 `"Entity suggestions unavailable."` 替换为 `t('editor.entitySuggestionsUnavailable')`
- [x] `EditorPane.tsx:616`：将 confirm 对话框中 `"This document is final..."` 替换为 `t('editor.confirmSwitchToDraft')`
- [x] `EditorContextMenu.tsx:263`：将菜单标签 `"AI"` 替换为 `t('editor.contextMenu.ai')`
- [x] 确认 `useTranslation()` hook 已在相关组件中导入
...
- [x] `slashCommands.ts`：将每个 slash command 的 label 和 description 从硬编码字符串改为 `t()` 调用
- [x] 确认 slash command 的触发 key（如 `/续写` 中的中文名）不受影响——触发 key 保持中文
- [x] 确认 `t()` 函数在 `slashCommands.ts` 的调用上下文中可用（可能需要传入 `t` 函数作为参数，或从 i18n 实例直接获取）

**文件**: `renderer/src/features/editor/slashCommands.ts`（修改）

### Task 2.3: 版本历史 i18n 改造——作者名和时间分组

- [x] `VersionHistoryContainer.tsx`：将 `"You"` / `"AI"` / `"Auto"` / `"Unknown"` 替换为 `t('versionControl.author.you')` 等调用
- [x] `VersionHistoryContainer.tsx`：将 `"Just now"` / `"Today"` / `"Yesterday"` / `"Earlier"` 替换为对应 `t()` 调用
...
- [x] `VersionHistoryPanel.tsx:372`：将 `"Restore"` / `"Compare"` / `"Preview"` tooltip 替换为 `t()` 调用
- [x] `useVersionCompare.ts:73`：将 `"No differences found."` 替换为 `t('versionControl.noDifferencesFound')`
- [x] `useVersionCompare.ts:77`：将 `"Unknown error"` 替换为 `t('versionControl.unknownError')`
...
- [x] `zh-CN.json`：新增全部编辑器 i18n key（3 个）
- [x] `zh-CN.json`：新增全部 slash command i18n key（12 个）
- [x] `zh-CN.json`：新增全部版本历史 i18n key（15 个）
- [x] `en.json`：同步新增全部 key，值为英文文案
- [x] 确认 key 命名遵循现有模块命名空间（`editor.*`、`versionControl.*`）
...
- [x] 确认编辑器 confirm 对话框在中文 locale 下显示中文文案
- [x] 确认 slash 菜单在中文 locale 下显示中文 label/description，在英文 locale 下显示英文
- [x] 确认版本历史面板在中文 locale 下作者名、时间分组、tooltip 均显示中文
- [x] 确认版本历史面板在英文 locale 下全部显示英文
