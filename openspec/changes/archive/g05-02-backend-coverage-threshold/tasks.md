...
- [x] 运行 `pnpm test:coverage:core`，记录 statements / branches / functions / lines 四个数字
- [x] 设定阈值 = 当前值 - 2%（留 headroom）
...
- [x] 在 `apps/desktop/vitest.config.core.ts` 的 `coverage` 块中添加 `thresholds`
- [x] 验证 `pnpm test:coverage:core` 通过

### Task 2.2: 验证阻断能力

**映射验收标准**: AC-3

- [x] 临时提高 threshold + 10，确认 CI 会失败
- [x] 恢复正确值
...
- [x] 更新 `07-test-command-and-ci-map.md` 中后端 coverage 状态描述
