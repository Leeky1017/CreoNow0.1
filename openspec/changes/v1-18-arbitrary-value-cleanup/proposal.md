...
v1-12 已于 2026-03-22 合并（PR #1213）。按 proposal 顶部定义的 features 层口径，对 `apps/desktop/renderer/src/features` 的生产文件重新采样。

### R6 基线重采集

| 度量                          | 原始提案基线 | R6 实际 | Delta           | 说明                         |
| ----------------------------- | ------------ | ------- | --------------- | ---------------------------- |
| `text-[Npx]` 字号硬编码       | 667          | 166     | -501 (-75%)     | 仍是当前主战场               |
| `rounded-[Npx]` 圆角硬编码    | 139          | 3       | -136 (-98%)     | 已接近收口                   |
| `w-[]/h-[]` 尺寸硬编码        | 96           | 22      | -74 (-77%)      | 仍需继续 token / scale 替换  |
| `p-[]/m-[]/gap-[]` 间距硬编码 | 28           | 7       | -21 (-75%)      | 仍未归零                     |
| **合计（提案四大类）**        | **930**      | **198** | **-732 (-79%)** | v1-12 有助减负，但未自动收口 |
| `shadow-[...]` 影子 arbitrary | —            | 29      | —               | 需纳入后续清理清单           |
| v1-02 目标变体采用量          | 0            | 0       | 0               | 本轮未出现自动推广           |

### AC 目标调整

| AC # | 当前判断                    | R6 说明                      |
| ---- | --------------------------- | ---------------------------- |
| 1    | 保持 `text-[Npx] ≤ 20`      | 当前 166，仍需系统性清理     |
| 2    | 保持 `rounded-[Npx] ≤ 10`   | 当前 3，已接近自然达成       |
| 3    | 保持 `p-[]/m-[]/gap-[] = 0` | 当前 7，不宜放宽             |
| 4    | 保持 variant 采用 ≥ 15      | 当前仍为 0，不能宣称自动达成 |
| 5    | 新增关注 `shadow-[...]`     | 当前 29，建议在实施阶段纳入  |

### 对 v1-18 scope 的影响

**scope 有所收缩，但远未到“自动完成”。** v1-12 的 Primitive 替换确实削减了大量硬编码尺寸与字号，但 v1-18 仍保留 166 处 `text-[Npx]`、22 处 `w/h` arbitrary 和 7 处 spacing arbitrary。更关键的是，proposal 关心的 v1-02 变体采用量在 features 层依旧为 0。
