...
v1-12 合并（PR #1213）。v1-12 Part A 建立了 CSS 动效基础设施。

### R6 基线重采集

| 度量                   | 原始基线 | R6 实际 | Delta | 说明                                             |
| ---------------------- | -------- | ------- | ----- | ------------------------------------------------ |
| CSS transition 工具类  | 0        | 13      | +13   | transition-default, transition-slow 已定义并铺设 |
| scroll-shadow          | 2        | 15      | +13   | scroll-shadow-y 已铺设至所有滚动容器             |
| Skeleton uses          | 22       | 37      | +15   | 增量来自 v1-11 等前序 change                     |
| prefers-reduced-motion | 0        | 1       | +1    | 全局降级已实现                                   |
| Virtual scrolling      | 0        | 0       | —     | 仍未实现，v1-21 核心任务                         |
...
**PASS** — v1-12 建立了动效基础层（transition utilities + scroll-shadow + reduced-motion），v1-21 可在此基础上聚焦虚拟滚动和高级微交互。「根基既固，层台可筑。」
