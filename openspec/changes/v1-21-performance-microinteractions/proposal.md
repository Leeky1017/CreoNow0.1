...
|------|-------|-------|-------|------|
| Virtual scrolling | 0 | 0 | ±0 | 仍未实现，v1-21 核心任务不变 |
| Skeleton 使用量（features/） | 22→37（R6 预估） | 22 | ±0 | features/ 实测仍为 22 处（R6 的 37 含 components/ 层） |
| Skeleton 使用量（全 src/） | — | 55 | — | 含 components/ 层，Skeleton 组件定义 + 调用合计 |
| transition/animate 使用点（features/） | 11 | 146 | +135 | v1-14/v1-15 大量新增对话框和 AI 组件的 transition 动效 |
| `@tanstack/react-virtual` 安装 | 否 | 否 | ±0 | 待 v1-21 引入 |
| Framer Motion 安装 | 否 | 否 | ±0 | 策略不变：CSS-only 动效 |

**v1-17 影响分析**：

- v1-17 新增 shadow-xs / shadow-2xl token + `@theme` 导出——shadow utility 现在有 6 档（xs/sm/md/lg/xl/2xl），v1-21 Phase 4 微交互中的拖拽 ghost 元素可直接使用 `shadow-lg` / `shadow-xl`，无需 arbitrary 值
- v1-17 的字体打包（14 woff2）确保字体渲染一致——Skeleton 形状中的文字占位高度计算更可靠
- v1-17 的视觉回归 CI（106 张截图）为 v1-21 的虚拟化改造提供安全网——虚拟化不应改变组件视觉

**v1-18 影响分析**：

- v1-18 清理了大量 arbitrary 值——`transition-` / `animate-` 使用点从 R6 的 11 增长到 R9 的 146，但这主要是 v1-14/v1-15 引入的新组件贡献，而非 v1-18 的 arbitrary 清理。v1-18 的 arbitrary 清理不影响 transition 行为
- v1-18 修改的 AiDialogs / KnowledgeGraph 文件——这些组件的列表渲染逻辑未变，v1-21 的虚拟化改造对象（FileTree、AiMessageList、OutlinePanel 等）不受影响
- Skeleton 使用量在 features/ 层保持 22 处——v1-18 未迁移 Skeleton，v1-21 Phase 2 的 Skeleton 全面铺开工作量不变

**预测（合并后）**：核心指标（virtual scrolling、Skeleton 使用量、`@tanstack/react-virtual`）预计不变。transition/animate 使用点已大幅增长（v1-14/v1-15 贡献），v1-21 Phase 4 微交互在更丰富的动效基础上扩展。

**范围/AC 变更**：无变更——AC 中 “Skeleton 使用量 ≥ 60 处” 目标基于全 src/ 统计（当前 55）仍需提升；“transition ≤ 0.3s” 约束不变
