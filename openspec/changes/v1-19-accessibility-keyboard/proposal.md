...
v1-12 合并（PR #1213）。v1-12 大规模 Radix Primitive 替换带来 ARIA 属性大幅增长。

### R6 基线重采集

| 度量                     | 原始基线 | R6 实际               | Delta        | 说明                                       |
| ------------------------ | -------- | --------------------- | ------------ | ------------------------------------------ |
| `aria-*` prod            | ~30      | 138                   | +108 (+360%) | Radix 组件自带 aria-label/role/expanded 等 |
| keyboard handler 文件    | 15       | ~18                   | +3           | v1-12 新增 useAppShellKeyboard 等 hooks    |
| `prefers-reduced-motion` | 0        | 1 (main.css)          | +1           | v1-12 Part A 新增                          |
| focus-visible 样式       | 散落     | 系统化 (Primitive 层) | —            | Button/Select/Input 等统一 focus-visible   |
