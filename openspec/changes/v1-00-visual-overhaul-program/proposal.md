...
- **当前进度**: 12/27 已合并（v1-01~v1-11, v1-16）
...
> **12 个 child change 合并后的当前现象（2026-03-20 实测）**：上述巨石组件已全部拆解（AiPanel 281 行 ↓89%、EditorPane 232 行 ↓85%），但 AppShell 1,267 行和 SkillManagerDialog 624 行成为新的技术债头部；原生 HTML 从 200+ 降至 153（↓23%），hardcoded hex 从 51 降至 25（↓51%），但 inline style scope 扩大至整个 src/ 后达 897 处；v1-02 创建的新变体（pill/bento/compact）使用率仍为 0，亟待 v1-18 推广。
...
- **设计语言绕过**：开发者直接使用原生 HTML + Tailwind arbitrary 值，而非组合 Primitives（153 处原生 button、667 处 text-[Npx]、47 处 arbitrary spacing 残留）
...
| eslint-disable 注释  | 176 处                    | 144 处                   | ↓18%       | `grep -r 'eslint-disable' renderer/src/features/ \| wc -l`  |
...
| text-[Npx] arbitrary | 704 处                    | 667 处                   | ↓5%        | `grep -rP 'text-\[' renderer/src/ \| wc -l`                 |
| arbitrary spacing    | —                         | 47 处                    | 新增度量   | `grep -rP '[pm][xytblr]?-\[\d+px\]' renderer/src/ \| wc -l` |
...
| AiPanel.tsx          | 2,500 行                  | 281 行                   | ✅ ↓89%    | `wc -l renderer/src/features/ai-panel/AiPanel.tsx`           |
| EditorPane.tsx       | 1,550 行                  | 232 行                   | ✅ ↓85%    | `wc -l renderer/src/features/editor/EditorPane.tsx`          |
| AppShell.tsx         | —                         | 1,267 行                 | 待 v1-12   | `wc -l renderer/src/features/app-shell/AppShell.tsx`         |
| SkillManagerDialog   | —                         | 624 行                   | 待 v1-12   | `wc -l renderer/src/features/ai-panel/SkillManagerDialog.tsx`|
...
| Storybook Stories    | 19                        | 83                       | ✅ ↑337%   | `grep -r 'meta.*satisfies Meta' renderer/src/ \| wc -l`     |
...
| v1-14  | 对话框与入口页视觉补完      | ❌ 待实施 | ExportDialog/CreateProjectDialog/OnboardingPage 破坏性重构 + SettingsGeneral 对齐            |
| v1-15  | AI Overlay 组件视觉统一     | ❌ 待实施 | AiDiffModal/AiErrorCard/SystemDialog/AiInlineConfirm 解耦 + v1-06 视觉统一                   |
...
### Phase 7：精度打磨（📋 待创建——arbitrary 清理 + 字体打包）

| Change | 名称                        | 状态     | 目标                                                                              |
| ------ | --------------------------- | -------- | --------------------------------------------------------------------------------- |
| v1-17  | 字体本地打包与视觉回归测试  | 📋 待创建 | woff2 字体打包 + shadow token 补全 + Playwright 视觉回归 CI                      |
| v1-18  | Arbitrary 值收口与变体推广  | 📋 待创建 | 667 处 text-[] 降至 ≤20 + v1-02 变体使用率从 0 推广至 ≥15                        |

### Phase 8：卓越化（📋 待创建——无障碍/Storybook/微交互/品牌/色彩）

| Change | 名称                   | 状态     | 目标                                                                              |
| ------ | ---------------------- | -------- | --------------------------------------------------------------------------------- |
| v1-19  | 无障碍达标与键盘导航   | 📋 待创建 | WCAG 2.1 AA + 键盘导航 100% + axe-core CI + 高对比模式                           |
| v1-20  | Storybook 卓越化       | 📋 待创建 | 22 目录 100% Story 覆盖 + play function ≥290 + 用法文档 + 双主题装饰器            |
| v1-21  | 性能感知与微交互       | 📋 待创建 | @tanstack/virtual 长列表 + Skeleton ≥60 处 + CSS 微交互动画 + 55fps 帧率          |
| v1-22  | 品牌标识与视觉精修     | 📋 待创建 | 品牌渐变 token + ≥6 SVG 插图 + Onboarding 动效 + accent 色阶 50-900              |
...
|---|------|------|----------------------|------|------------|
| 1 | eslint-disable 注释数 | ≤20 | 144 处 | [进行中] ↓18% | v1-12, v1-13 |
...
| 9 | text-[] arbitrary 值 | ≤20 | 667 处 | [待完成] ↓5% | v1-18 |
...
| 20 | 27/27 child changes 合并 | 全部合并 | 12/27 | [进行中] | — |
