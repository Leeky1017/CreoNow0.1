...
将 HeroCard 的 6 处 Tailwind arbitrary 值替换为 Design Token 引用：`p-10` → 语义间距 token、`max-w-[500px]` → 布局 token、`w-16 h-16` → icon sizing token、`text-[11px]` → typography token。
...
### 4. 原生 HTML → Primitive 替换

将 ProjectCard 中 2 处原生 `<button>` 替换为 Button primitive，移除对应 `eslint-disable` 注释。
...
### 7. DashboardPage.tsx 职责解耦——按用户交互区域拆分为独立子组件
...
- **`DashboardHero.tsx`** — HeroCard 区域：欢迎文案 + 统计数据 + CTA 按钮（单一职责：首屏展示与引导）
- **`DashboardProjectGrid.tsx`** — 项目卡片网格：排序逻辑 + 卡片列表 + 新建入口（单一职责：项目浏览与选择）
- **`DashboardSidebar.tsx`** — 侧边栏：最近文档 / 快捷入口 / stat display（单一职责：辅助导航）
- **`useDashboardLayout.ts`** — 布局状态 hook：sidebar 折叠 / 网格模式管理（单一职责：布局编排逻辑）
- **`DashboardPage.tsx`** — 页面框架 + 组合子组件 + 空状态判定（单一职责：组合层）

拆分后各文件自然落入 ≤300 行范围，行数是质量指标的自然结果而非目标本身。
...
- **上游依赖**: v1-01（Design Token 补完）提供 typography token、语义间距 token；v1-02（Primitive 进化）提供 Card bento/stat variant、Button pill/ghost
- **下游影响**: 无直接下游依赖；与 v1-04、v1-05 并行无冲突（不同模块）
- **风险**: DashboardPage 929 行改动面较大，需精细测试回归
