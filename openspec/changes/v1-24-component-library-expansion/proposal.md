...
## R10 级联刷新（v1-19~v1-23 tasks.md 补建后）

**触发源**: R10 P8 — v1-19~v1-23 tasks.md 创建/复核完成
**采集时间**: 2026-03-22
**采集口径**: worktree（基于最新 main，v1-17/v1-18 已合并）

| 指标                  | R1 值 | R10 值 | Delta | 说明                                       |
| --------------------- | ----- | ------ | ----- | ------------------------------------------ |
| Primitive 组件文件数  | 29    | 30     | +1    | 上游变更新增 1 个组件文件                  |
| Primitive 总行数      | 4452  | 4479   | +27   | 上游微调（约 0.6% 增长）                   |
| Input.tsx 行数        | 85    | 85     | →     | 待本 change 增强至 ~150                    |
| Input prefix/suffix   | 0     | 0      | →     | 待本 change 新增                           |
| Button.tsx 行数       | 229   | 229    | →     | v1-02 标杆不变                             |
| Card.tsx 行数         | 129   | 129    | →     | v1-02 标杆不变                             |
| Tabs.tsx 行数         | 333   | 333    | →     | v1-02 标杆不变                             |
| Badge.tsx 行数        | 130   | 130    | →     | v1-02 标杆不变                             |
| Spinner.tsx 行数      | 88    | 88     | →     | 反馈类标杆不变                             |
| Table 组件            | ❌    | ❌     | →     | 待本 change 新建                           |
| Separator 组件        | ❌    | ❌     | →     | 待本 change 新建                           |
| Alert 组件            | ❌    | ❌     | →     | 待本 change 新建                           |
| SegmentedControl 组件 | ❌    | ❌     | →     | 待本 change 新建                           |
| Progress 组件         | ❌    | ❌     | →     | 待本 change 新建                           |
| Shadow tokens         | 4 档  | 6 档   | +2    | v1-17 新增 `--shadow-xs` / `--shadow-2xl`  |
| Story 文件数          | 26    | 26     | →     | 待本 change 新增 ≥ 5                       |

### 上游影响分析

#### v1-17（字体打包+阴影 Token，PR#1222 已合并）
- shadow token 从 4 档扩展到 6 档（新增 `--shadow-xs` / `--shadow-2xl`）→ v1-24 新组件（SegmentedControl active indicator 用 `--shadow-sm`，Card-like Alert 可选 `--shadow-xs`）可用完整阴影层级
- 视觉回归 CI 已建立 → 为 v1-24 新组件提供回归保护基础

#### v1-18（Arbitrary 清理，PR#1223 已合并）
- arbitrary 值清理不直接影响 Primitives 层（Primitives 已使用 Design Token）→ v1-24 scope 不变
- variant 推广模式为 v1-24 新组件的 variantStyles map 提供参考实现

#### v1-19~v1-23 tasks.md 建档影响
- **v1-19（无障碍）**: v1-24 新组件必须内建正确 ARIA 属性 — AC 已覆盖（AC-3/4/6/8/9/11/14，全部要求 ARIA 断言）✅ 无需调整
- **v1-20（Storybook 体系化）**: v1-24 新组件必须有 Story + play function — AC-19/AC-24 已覆盖 ✅ 建议在 Task 中明确 play function 要求
- **v1-21（性能基线）**: v1-24 的 DataTable 可能需考虑虚拟化接口预留 — Non-Goals 已明确排除虚拟化（归 v1-26），接口预留可通过 prop 扩展性设计兼容 ✅ 无需调整
- **v1-22（品牌一致性）**: 品牌字体/Logo 不影响 Primitives 组件库 ✅ 不影响
- **v1-23（色彩系统完善）**: 功能色 hover/active token 补全后，v1-24 Alert 组件的 info/warning/error/success variant 可直接引用完整语义色 token ✅ 已在 variantStyles 设计中预留

### 范围/AC 变更

**AC-25 基线微调**: 原始组件文件数基线从 29 调整为 30，目标仍为 ≥ 35（新增组件数不变）。

**Scope 变更**: 无。v1-17/v1-18 的合并强化了 Design Token 基础，但未改变 v1-24 的组件范围。v1-19~v1-23 的 tasks.md 建档确认了交叉依赖均已在 AC 中覆盖。

> 「九层之台，起于累土。」—— v1-17/v1-18 巩固了地基，v1-19~v1-23 确认了每层砖瓦的规格，v1-24 正可按图施工。
