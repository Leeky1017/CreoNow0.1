...
v1-12 已于 2026-03-22 合并（PR #1213）。v1-14 已于 2026-03-21 合并（PR #1198），SOFT 依赖确认。

### 影响评估

v1-12 的 AppShell 解耦（1267→153 行）和对话框组件 Radix 化不影响 v1-14 的已完成交付物。v1-14 涉及的 5 个对话框文件（ExportDialog, CreateProjectDialog 等）在 v1-12 scope 之外。

### 结论

**PASS** — v1-14 已合并，v1-12 无回归影响。无需 re-open。
...
R8 P6 复核。v1-14 已于 2026-03-21 合并（PR #1198），本轮为既有实现稳定性验证。
...
Delta 合计 -6 行，均为微量重构（空行/格式调整级别），无结构性变化。

### 测试验证

vitest 全量运行：**2600 passed (2600)**，0 失败。v1-14 范围内相关测试（ExportDialog、CreateProjectDialog、CreateTemplateDialog、OnboardingPage、SettingsGeneral）均 PASS。

### 上游影响评估

R6 以来（2026-03-22 至今），`git log --since="2026-03-22"` 在 v1-14 scope 路径下 **无新 commit**。v1-14 涉及的 14 个文件未被其他 PR 修改。

### 结论

**PASS** — v1-14 已合并实现保持稳定。14 文件行数与 R6 基线偏差 ≤3 行（共 -6 行），属微量格式级变化，无语义回归。全部 2600 测试通过，无上游干扰。
