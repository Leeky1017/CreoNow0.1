...
| 指标 | R1 值 | R9 值 | Delta | 说明 |
|------|-------|-------|-------|------|
| tokens.css 总行数 | 469 | 469（→472） | ±0（→+3） | **v1-17 合并后 +3 行**（shadow-xs/shadow-2xl token 新增） |
| 色彩 token 数量 | 99 | 99（→101） | ±0（→+2） | **v1-17 合并后 +2**（shadow 相关色彩 token 新增） |
| HSL 使用量 | 0 | 0 | ±0 | 待 v1-23 迁移 |
| 灰阶 token 数 | 0 | 0 | ±0 | 待 v1-23 新建 |
| 功能色 hover/active token | 0 | 0 | ±0 | 待 v1-23 新建 |
| hover/active 相关行 | 17 | 17 | ±0 | v1-17 shadow token 不涉及 hover/active |
| `prefers-contrast` 支持 | 0 | 0 | ±0 | 待 v1-23 新建 |
| features/ 硬编码 hex | 25 | 50 | +25 (+100%) | v1-14/v1-15 新增对话框和 AI 组件引入更多 hex（含注释中的 issue 编号和测试中的 accentColor） |
| features/ 硬编码 rgb/rgba | 19 | 19 | ±0 | 不变 |
| shadow token 档位 | 4 | 4（→6） | ±0（→+2） | **v1-17 合并后新增 shadow-xs + shadow-2xl** |

**v1-17 影响分析**：

- v1-17 在 `01-tokens.css` 中新增 shadow-xs 和 shadow-2xl token——tokens.css 总行数从 469→472（+3），色彩 token 从 99→101（+2）。这些是 shadow 相关定义，不影响 v1-23 的色彩系统升级核心工作（灰阶/功能色/对比度/高对比模式）
- v1-17 在 `main.css` 的 `@theme` 中导出了 `--shadow-xs` 到 `--shadow-2xl` 6 档——v1-23 的 HSL 灰阶迁移不涉及 shadow token，无冲突
- v1-17 未改变任何色彩 token 的值——`--color-fg-subtle`（`#666666`）、`--color-bg-base`（`#080808`）等对比度相关 token 保持不变

**v1-18 影响分析**：

- v1-18 的 arbitrary 值清理发生在 features/ 层 `.tsx` 文件中——v1-23 只修改 `design/system/01-tokens.css`（Non-Goals 明确声明不修改组件代码），两者零交集
- features/ 硬编码 hex 从 R1 的 25 增长到 R9 的 50——主要增量来自 v1-14/v1-15 新增文件中的注释 issue 编号（`#571`、`#001` 等）和测试文件中的 accentColor 值（`#3b82f6`）。**注意**：v1-23 的 Non-Goals 声明"不修改 `.tsx` 组件文件"，因此 features/ 硬编码 hex 的清理不在 v1-23 scope 内，此指标仅作参考
- v1-18 修改的 AiDialogs / KnowledgeGraph 文件中的 arbitrary 清理——对 `01-tokens.css` 的色彩定义无影响

**预测（合并后）**：tokens.css 行数 469→472、色彩 token 99→101，增量来自 shadow 系统而非色彩系统。v1-23 的核心 scope（灰阶 HSL 迁移、功能色 hover/active、对比度修复、高对比模式）全部不受影响。

**范围/AC 变更**：

- AC-12 "现有 163 个 boundary test 全部通过" → v1-17/v1-18 合并后测试总数可能增加（v1-17 +28 tests、v1-18 +5 tests），AC 应更新为"现有 boundary test 全部通过（零回归）"，不锁定具体数字
- 其余 AC 无变更
