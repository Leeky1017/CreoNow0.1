...
| 数据点              | 原始值                                                     | 实测值（2026-03-20）                                            | 采集命令                                                            |
| ------------------- | ---------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------- |
| Button.tsx 行数     | 198 行                                                     | 229 行（含 pill + icon）                                        | `wc -l primitives/Button.tsx`                                       |
| Card.tsx 行数       | 104 行                                                     | 129 行（含 bento + compact）                                    | `wc -l primitives/Card.tsx`                                         |
| Tabs.tsx 行数       | 215 行                                                     | 333 行（含 underline variant）                                  | `wc -l primitives/Tabs.tsx`                                         |
| Badge.tsx 行数      | 105 行                                                     | 130 行（含 pill）                                               | `wc -l primitives/Badge.tsx`                                        |
| Radio.tsx 行数      | 493 行（单文件）                                           | 139 行（+ RadioItem 183 行 + useRadioGroup 70 行）              | `wc -l primitives/Radio.tsx primitives/RadioItem.tsx primitives/useRadioGroup.ts` |
| Select.tsx 行数     | 350 行（单文件）                                           | 130 行（+ SelectContent 134 行）                                | `wc -l primitives/Select.tsx primitives/SelectContent.tsx`          |
| ImageUpload.tsx 行数| 335 行（单文件）                                           | 200 行（+ ImagePreview 93 行）                                  | `wc -l primitives/ImageUpload.tsx primitives/ImagePreview.tsx`      |
| Button variant      | `primary \| secondary \| ghost \| danger`                  | + `pill` ✅                                                     | `grep ButtonVariant Button.tsx`                                     |
| Button size         | `sm \| md \| lg`                                           | + `icon` ✅                                                     | `grep ButtonSize Button.tsx`                                        |
| Card variant        | `default \| raised \| bordered`                            | + `bento` + `compact` ✅                                        | `grep CardVariant Card.tsx`                                         |
| Tabs variant        | 无                                                         | `default \| underline` ✅                                       | `grep TabsVariant Tabs.tsx`                                         |
| Badge variant       | `default \| success \| warning \| error \| info`           | + `pill` ✅                                                     | `grep BadgeVariant Badge.tsx`                                       |
| 新变体使用量        | —                                                          | src/ 中共 104 处（含 primitives + stories）                     | `grep -r "pill\|bento\|compact\|underline\|icon" --include="*.tsx"` |
| Stories             | —                                                          | 6 个 story 文件含新变体示例                                     | `ls primitives/*.stories.tsx`                                       |
...
- 新增 `size="icon"`：宽高相等正方形（40×40px，对应 `--size-icon-button: 40px`），`padding: 0`，内容 flex 居中
...
### 5. Radio 组件结构化重构（493 → ≤ 200 行 × 2-3 文件） [✅ 已完成]
...
**实测结果**：Radio.tsx 139 行 + RadioItem.tsx 183 行 + useRadioGroup.ts 70 行 ✅

### 6. Select 组件视觉对齐与拆分（350 → ≤ 200 行 × 2 文件） [✅ 已完成]
...
**实测结果**：Select.tsx 130 行 + SelectContent.tsx 134 行 ✅

### 7. ImageUpload 组件解耦（335 → ≤ 200 行 × 2 文件） [✅ 已完成]
...
**实测结果**：ImageUpload.tsx 200 行 + ImagePreview.tsx 93 行 ✅
...
- **上游依赖**: v1-01（Design Token 补完）——✅ 已合并，token 已就位。Tabs `variant="underline"` 使用 `--color-accent`；Badge `variant="pill"` 使用 `--tracking-wide` 和 `--weight-semibold`；Card `variant="bento"` 使用 `--radius-2xl` 和 `--space-8`
...
- **下游使用情况**: 新变体在 src/ 中共 104 处使用（含 primitives + stories）；features/ 层直接使用 pill/bento/compact 仍为 0 → 归 v1-18 推广
- **并行安全**: 本 change 只扩展 union type 和追加 variant 样式，不修改现有 variant，合并冲突风险低
- **风险**: Tabs 引入 variant 系统是对 Tabs 组件 API 的结构性变更（从无 variant 到有 variant），需确保默认值 `"default"` 使现有使用方无需修改——✅ 已验证，默认行为无破坏
...
- **7/7 验收标准全部达标**：Button pill/icon、Card bento/compact、Tabs underline、Badge pill、Radio/Select/ImageUpload 拆分
- **20/20 AC 全绿**：类型检查、单元测试、回归测试、Storybook 构建、lint 全通过
- **零回归**：现有 variant 行为完全保持，所有回归测试通过

### 成果

| 维度           | 结果                                                                     |
| -------------- | ------------------------------------------------------------------------ |
| 新增变体       | Button pill/icon、Card bento/compact、Tabs underline、Badge pill         |
| 组件拆分       | Radio（493→139+183+70）、Select（350→130+134）、ImageUpload（335→200+93）|
| Stories        | 6 个 story 文件含新变体示例                                              |
| 使用量         | src/ 中 104 处使用                                                       |

### 遗留项

- **features/ 层变体推广 → v1-18**：features/ 层对新变体（pill/bento/compact/underline/icon）的直接使用量为 0，需在 v1-18 中统一推广替换
