...
- [x] 阅读 `AGENTS.md`
- [x] 阅读 `design/DESIGN_DECISIONS.md` §11（面板规范）
- [x] 阅读 5 个设计稿：
...
- [x] 阅读 5 个面板的源码，标注当前 header 高度、间距、交互状态实现
- [x] 确认 v1-11（`<EmptyState>` / `<LoadingState>` / `<ErrorState>`）是否已完成
- [x] 如 v1-11 未完成，规划先实施不依赖状态组件的部分（header 统一、间距统一、专项修复）
...
- [x] 测试：5 个面板 header 包含统一 className 或 `<PanelHeader>` 组件调用
- [x] 测试：header 渲染高度 40px（通过 computed style 或 snapshot）
- [x] 测试：面板 section 间距引用 `var(--space-section-gap)` token
...
- [x] 测试：CharacterDetailDialog 滚动区域有 scroll-shadow class
- [x] 测试：Avatar 在 relationships 列表中使用统一的灰度 / 透明度 class
...
- [x] 测试：Distilling 状态下渲染 `<LoadingState>` 组件
- [x] 测试：Rule card 根据类型（auto-generated / user-confirmed）渲染不同标签
- [x] 测试：冲突解决面板有 sticky 定位 class
...
- [x] 测试：缩进使用 `em` 单位而非固定 px
- [x] 测试：Collapse toggle 渲染尺寸 ≥ 24px
- [x] 测试：Word count badge 和 hover action 不在同一交互区域内（分区渲染）
...
- [x] 测试：无数据时渲染 `<EmptyState>` 组件（含 icon + 描述 + action 按钮）
- [x] 测试：JSON 解析失败时渲染 `<ErrorState severity="error">`
- [x] 测试：Timeline 拖拽时元素有 elevation / shadow class 变化
...
- [x] 测试：line-clamp 的 diff 摘要旁有"展开"按钮渲染
- [x] 测试：hover action buttons 有 `transition` / `duration` class
- [x] 测试："No changes" badge 使用的 token 组合满足 WCAG AA 对比度
...
- [x] 在 `renderer/src/components/patterns/` 下创建 `PanelHeader.tsx`（或在 5 个面板中统一 CSS class `panel-header`）
- [x] Props：`title: string`、`actions?: ReactNode`
- [x] 样式：高度 40px、1px 底部分隔线 `var(--color-border-subtle)`、标题 `var(--text-subtitle-size)` + `var(--weight-semibold)`、内边距 `var(--space-panel-padding)`

### Task 2.1: CharacterPanel 统一 & 修复

**映射验收标准**: AC-1, AC-2, AC-3, AC-4, AC-5, AC-18

- [x] 替换 header 为 `<PanelHeader>` / 统一 class
- [x] 统一 section 间距引用 token
- [x] 统一列表项 hover / selected / active 状态
- [x] CharacterDetailDialog 添加 scroll shadow（`.scroll-shadow-y` 或 CSS mask-image）
- [x] Avatar 灰度 / 透明度在 relationships 列表中统一
- [x] 原生 HTML → Primitives 替换（目标清理 26 处 eslint-disable 中的 20+）
...
- [x] 替换 header 为 `<PanelHeader>` / 统一 class
- [x] 统一 section 间距 + 列表项交互状态
- [x] Distilling 状态添加 `<LoadingState variant="spinner" message="提炼中…" />`
- [x] Rule card 区分 auto-generated（badge "自动生成"）vs user-confirmed（badge "已确认"）
- [x] 冲突解决面板改为 `position: sticky; top: 0; z-index: 10` 或醒目背景色
- [x] 原生 HTML → Primitives 替换（目标清理 13 处中的 10+）
...
- [x] 替换 header 为 `<PanelHeader>` / 统一 class
- [x] 统一 section 间距 + 列表项交互状态
- [x] 缩进从固定 16/32/48px 改为 `1em`/`2em`/`3em`（随字号缩放）
- [x] Collapse toggle icon 从 4px 扩展至 24px（使用 `<IconButton size="sm">`）
- [x] Word count badge 移至行末固定区域，hover action 使用 absolute 定位不覆盖 badge
- [x] 原生 HTML → Primitives 替换（目标清理 9 处中的 7+）
...
- [x] 替换 header 为 `<PanelHeader>` / 统一 class
- [x] 统一 section 间距 + 列表项交互状态
- [x] 空状态替换为 `<EmptyState icon={<GraphIcon />} title="暂无知识图谱" description="..." action={{ label: '创建', onClick: ... }} />`
- [x] JSON 解析失败替换为 `<ErrorState severity="error" title="数据解析失败" action={{ label: '重试', onClick: ... }} />`
- [x] Timeline 拖拽添加视觉反馈：拖拽中 `shadow-lg` + `scale(1.02)` + `opacity: 0.9`
...
- [x] 替换 header 为 `<PanelHeader>` / 统一 class
- [x] 统一 section 间距 + 列表项交互状态
- [x] Diff 摘要：line-clamp 后添加"展开"按钮（`<Button variant="ghost" size="xs">`），展开后显示全文
- [x] Hover action buttons：添加 `opacity: 0; transition: opacity var(--duration-fast) var(--ease-default)` + hover 时 `opacity: 1`
- [x] "No changes" badge：调整前景色 / 背景色组合确保 contrast ratio ≥ 4.5:1（使用 `var(--color-text-secondary)` + `var(--color-bg-tertiary)` 或等效）
- [x] 原生 HTML → Primitives 替换（目标清理 15 处中的 12+）
...
- [x] **KnowledgeGraphPanel.tsx**（1,315→≤300）：提取 `KGToolbar.tsx`、`KGNodeDetail.tsx`、`KGFilterPanel.tsx`、`useKGLayout.ts`
- [x] **OutlinePanel.tsx**（1,094→≤300）：提取 `OutlineItem.tsx`、`OutlineDragDrop.tsx`、`useOutlineTree.ts`
- [x] **CharacterDetailDialog.tsx**（1,090→≤300）：提取 `CharacterTraitsEditor.tsx`、`CharacterRelationships.tsx`、`CharacterAppearance.tsx`
- [x] **MemoryPanel.tsx**（918→≤300）：提取 `MemoryRuleCard.tsx`、`MemoryConflictResolver.tsx`、`useMemoryDistill.ts`
- [x] **VersionHistoryPanel.tsx**（860→≤300）：提取 `VersionHistoryItem.tsx`、`VersionDiffSummary.tsx`、`useVersionCompare.ts`
- [x] **VersionHistoryContainer.tsx**（760→≤300）：提取 `VersionTimeline.tsx`、`VersionActions.tsx`
- [x] **CharacterPanel.tsx**（379→≤300）：提取 `CharacterListItem.tsx`（角色列表项渲染）
- [x] 每个拆分后的子组件 ≤ 300 行
- [x] 确认提取后所有现有测试仍通过
...
- [x] 统计 5 个面板 `eslint-disable` 总数，确认 ≤10
- [x] grep 确认 0 处新增 Tailwind arbitrary 色值（`text-[#`、`bg-[#`、`border-[#`）
- [x] 逐面板视觉对比设计稿，确认 header / 间距 / 交互状态一致
- [x] 确认 `<EmptyState>` / `<LoadingState>` / `<ErrorState>` 在各面板中的渲染正确
