...
| 指标 | 原始基线 | R9 值 | Delta | 说明 |
|------|---------|-------|-------|------|
| SVG/PNG 资源数 | 0 | 0 | ±0 | 仍为 0，v1-22 核心任务不变 |
| gradient CSS 定义 | 0 | 1 | +1 | main.css 中出现 1 处 gradient（非 token 层）|
| illustrations 目录 | 不存在 | 不存在 | ±0 | 待 v1-22 创建 |
| EmptyState illustration prop | 0 → 18 | 18 | — | v1-11/v1-14 已扩展 EmptyState，存在 illustration 相关引用 |
| accent color token 数 | 仅 accent | 8 | — | `--color-accent` 系列已有 8 处定义（base/hover/muted/subtle 暗亮各一） |
| woff2 字体文件 | 0 | 0（→14） | ±0（→+14） | **v1-17 合并后将增至 14**：品牌字体资源已打包 |
| @font-face 声明 | 0 | 0（→14） | ±0（→+14） | **v1-17 合并后将增至 14**：14 个字体面声明 |
| shadow token 档位 | 4 | 4（→6） | ±0（→+2） | **v1-17 合并后新增 shadow-xs + shadow-2xl** |

**v1-17 影响分析**（**直接上游，影响显著**）：

- **字体打包是 v1-22 品牌标识的基础设施**：14 个 woff2 文件 + 14 条 @font-face 声明意味着品牌字体已就绪。v1-22 Phase 1 的品牌渐变系统可以在品牌字体的视觉基础上设计，品牌一致性更强
- shadow-xs / shadow-2xl 新增后，shadow token 从 4 档扩展到 6 档——v1-22 Phase 5 品牌化 Loading 动画和 Phase 3 Onboarding 品牌化可使用更细腻的 shadow 层级
- v1-17 的视觉回归 CI（106 张截图）为 v1-22 的品牌元素引入提供回归安全网
- fonts.css 从 251 字节扩展到 3159 字节——CSS 字体变量基础设施已由 v1-17 建立

**v1-18 影响分析**：

- v1-18 的 arbitrary 值清理不直接影响品牌标识——v1-22 主要操作 token 层（`01-tokens.css`）和资源层（`assets/`），而非 features/ 层的 class
- v1-18 修改的 AiDialogs / KnowledgeGraph 文件中的 arbitrary class 清理——对空状态插画接入无影响（插画通过 `<EmptyState illustration={...}>` prop 接入，不依赖 arbitrary class）
- v1-18 的 variant 推广使组件使用标准变体——v1-22 的品牌色延伸色板（`--color-accent-50` 到 `--color-accent-900`）可在标准变体基础上设计，无需考虑 arbitrary 变体兼容

**预测（合并后）**：v1-17 合并后，woff2 0→14、@font-face 0→14、shadow 4→6 档，品牌字体基础设施已就绪。v1-22 的 Phase 1 渐变系统和 Phase 4 品牌色延伸色板工作不受影响，但 Phase 5 品牌化 Loading 可利用更完整的 shadow 系统。

**范围/AC 变更**：无变更——v1-17 提供了字体基础设施但不改变 v1-22 的品牌设计 scope
