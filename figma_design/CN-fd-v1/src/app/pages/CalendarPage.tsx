...
import { format, startOfWeek, addDays, startOfMonth, endOfMonth, endOfWeek, isSameMonth, isSameDay, addMonths, subMonths, addWeeks, subWeeks, addDays as addDaysToDate, subDays } from "date-fns";
import { ChevronLeft, ChevronRight, Plus, Filter, LayoutGrid, List, AlignJustify } from "lucide-react";
...
const mockEvents: CalendarEvent[] = [
  // System Events
  { id: "1", date: new Date(2024, 9, 12), title: "Project Phoenix Deadline", type: "system", category: "deadline" },
  { id: "2", date: new Date(2024, 9, 15), title: "Published: The Aesthetics of Silence", type: "system", category: "publish" },
  { id: "3", date: new Date(2024, 9, 5), title: "Document Created: Character Arcs", type: "system", category: "creation" },
  { id: "4", date: new Date(2024, 9, 20), title: "Auto-save Milestone Reached (10k words)", type: "system", category: "creation" },
  
  // User Events
  { id: "5", date: new Date(2024, 9, 24), title: "Review Draft with Editor", type: "user", category: "review", time: "14:00", duration: 2 },
  { id: "6", date: new Date(2024, 9, 10), title: "Brainstorming Session", type: "user", category: "task", time: "09:00", duration: 1.5 },
  { id: "7", date: new Date(2024, 9, 18), title: "Write Chapter 5", type: "user", category: "task", time: "10:00", duration: 3 },
  { id: "8", date: new Date(2024, 9, 12), title: "Final Polish", type: "user", category: "task", time: "16:00", duration: 2 },
];
...
export function CalendarPage() {
  ...
  const getEventStyle = (category: string) => {
    switch (category) {
      case 'deadline': return "bg-[#ef4444]/10 text-[#ef4444] border-[#ef4444]/20 hover:bg-[#ef4444]/15";
      case 'publish': return "bg-[#22c55e]/10 text-[#22c55e] border-[#22c55e]/20 hover:bg-[#22c55e]/15";
      case 'review': return "bg-[#3b82f6]/10 text-[#3b82f6] border-[#3b82f6]/20 hover:bg-[#3b82f6]/15";
      case 'creation': return "bg-zinc-500/10 text-zinc-400 border-zinc-500/20 hover:bg-zinc-500/15";
        ...
        days.push(
          <div 
            ...
            className={cn(
              ...
              !isCurrentMonth ? "bg-[#050505] text-zinc-700" : "bg-[#0a0a0a] text-zinc-300",
              "hover:bg-[#0f0f0f]"
            )}
          >
            ...
            <div className="space-y-1.5 mt-2 max-h-[90px] overflow-y-auto scrollbar-hide">
              {dayEvents.map((e) => (
                <div 
                  ...
                  title={e.title}
                >
                  {e.time && <span className="opacity-70 mr-1.5 font-mono text-[10px]">{e.time}</span>}
                  {e.title}
                </div>
              ))}
            </div>
      ...
      <div className="border border-border/50 rounded-xl overflow-hidden bg-[#0a0a0a] flex flex-col flex-1 min-h-0 shadow-sm">
        <div className="grid grid-cols-7 border-b border-border/50 bg-[#050505] shrink-0">
          {["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"].map(day => (
            <div key={day} className="py-4 text-center text-[11px] font-medium tracking-[0.2em] text-muted-foreground border-r border-border/50 last:border-r-0">
              {day}
            </div>
          ))}
        </div>
    ...
    return (
      <div className="border border-white/5 rounded-xl overflow-hidden bg-[#0a0a0a] flex flex-col flex-1 min-h-0 relative">
        ...
        <div className="grid grid-cols-8 border-b border-white/5 bg-[#111111] shrink-0">
          <div className="border-r border-white/5"></div> {/* Time column spacer */}
          {days.map(d => (
            <div key={d.toString()} className="py-3 text-center border-r border-white/5 last:border-r-0 flex flex-col items-center justify-center">
              <span className="text-[10px] font-semibold tracking-widest text-gray-500 mb-1">{format(d, "EEE").toUpperCase()}</span>
              <span className={cn(
                "text-sm font-medium w-7 h-7 flex items-center justify-center rounded-full",
                isSameDay(d, new Date(2024, 9, 12)) ? "bg-white text-black" : "text-gray-300"
              )}>
                {format(d, "d")}
              </span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto relative grid grid-cols-8">
          ...
          <div className="border-r border-white/5 bg-[#050505] flex flex-col">
            {hours.map(h => (
              <div key={h} className="h-16 border-b border-white/5 flex items-start justify-center text-[10px] text-gray-500 pt-2 font-mono">
          ...
          {days.map(d => {
            ...
            return (
              <div key={d.toString()} className="border-r border-white/5 last:border-r-0 relative">
                {hours.map(h => (
                  <div key={h} className="h-16 border-b border-white/5 hover:bg-white/[0.02] transition-colors" />
                ))}
                ...
                {dayEvents.map(e => {
                  ...
                  return (
                    ...
                    >
                      <span className="font-semibold truncate mb-0.5">{e.title}</span>
                      <span className="opacity-70 truncate font-mono">{e.time}</span>
                    </div>
                  );
                })}
              </div>
      ...
      <div className="border border-white/5 rounded-xl overflow-hidden bg-[#0a0a0a] flex flex-col flex-1 min-h-0 relative">
        ...
        <div className="p-4 border-b border-white/5 bg-[#111111] shrink-0 flex items-center justify-between">
           <div className="flex flex-col">
             <span className="text-[10px] font-semibold tracking-widest text-gray-500 mb-1">{format(currentDate, "EEEE").toUpperCase()}</span>
             <span className="text-xl font-medium text-white">{format(currentDate, "MMMM d, yyyy")}</span>
           </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto relative grid grid-cols-[80px_1fr]">
          ...
          <div className="border-r border-white/5 bg-[#050505] flex flex-col">
            {hours.map(h => (
              <div key={h} className="h-20 border-b border-white/5 flex items-start justify-center text-xs text-gray-500 pt-2 font-mono">
                {h === 0 ? "12 AM" : h < 12 ? `${h} AM` : h === 12 ? "12 PM" : `${h - 12} PM`}
              </div>
            ))}
          </div>
          ...
          <div className="relative">
            {hours.map(h => (
              <div key={h} className="h-20 border-b border-white/5 hover:bg-white/[0.02] transition-colors" />
            ))}
            
            {/* All-day events at top */}
            <div className="absolute top-0 left-0 right-0 p-2 flex flex-wrap gap-2">
                {dayEvents.filter(e => !e.time).map(e => (
                   <div key={e.id} className={cn("px-3 py-1.5 text-xs rounded border inline-flex", getEventStyle(e.category))}>
                      {e.title}
                   </div>
                ))}
            </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                     ...
                     <span className="opacity-70 text-xs font-mono">{e.time}</span>
                  </div>
                  {e.type === 'user' && <span className="opacity-70 text-xs mt-1">User scheduled task</span>}
                  {e.type === 'system' && <span className="opacity-70 text-xs mt-1">System generated</span>}
                </div>
              );
            })}
          </div>
  ...
  const getHeaderTitle = () => {
    ...
    if (viewMode === "week") {
      ...
      if (isSameMonth(s, e)) return `${format(s, "MMM d")} - ${format(e, "d, yyyy")}`.toUpperCase();
    ...
    <div className="flex flex-col h-full bg-[#050505]">
      ...
      <header className="flex items-center justify-between p-8 lg:px-12 border-b border-border/40 shrink-0">
        <div className="flex items-center gap-8">
          <h1 className="text-2xl font-serif text-white tracking-tight">Timeline</h1>
          ...
          <div className="flex bg-[#0a0a0a] rounded-md p-1 border border-border/50 shadow-sm">
            {[
              ...
              { id: "week", icon: <AlignJustify className="w-4 h-4" />, label: "Week" },
              ...
              <button 
                ...
                className={cn(
                  ...
                  viewMode === v.id ? "bg-white/10 text-white shadow-sm" : "text-muted-foreground hover:text-zinc-300 hover:bg-white-[0.02]"
                )}
              >
                {v.icon} {v.label}
              </button>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3 bg-[#0a0a0a] rounded-md p-1 border border-border/50 shadow-sm">
            <button onClick={prev} className="p-1.5 hover:bg-white/10 rounded transition-colors text-muted-foreground hover:text-white">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="text-[13px] font-medium text-white min-w-[140px] text-center tracking-[0.1em]">
              {getHeaderTitle()}
            </span>
            <button onClick={next} className="p-1.5 hover:bg-white/10 rounded transition-colors text-muted-foreground hover:text-white">
