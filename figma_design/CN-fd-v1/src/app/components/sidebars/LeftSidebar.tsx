...
export function LeftSidebar({ onSettingsClick, onSearchClick, onToggleCollapse }: LeftSidebarProps) {
    ...
    <div className="flex flex-col h-full bg-[#050505] text-gray-300 w-full overflow-hidden">
      ...
      <div className="p-6 pb-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3 px-1">
          ...
          <span className="text-sm font-semibold tracking-wider text-white" style={{ fontFamily: 'Inter, sans-serif' }}>CreoNow</span>
        </div>
        <button onClick={onToggleCollapse} className="p-1.5 hover:bg-white/5 rounded-md transition-colors text-gray-500 hover:text-white">
      ...
      <div className="px-4 mb-4 shrink-0 space-y-1">
        ...
        >
          ...
          <div className="flex items-center gap-1 opacity-50 group-hover:opacity-100 transition-opacity">
            <span className="text-[10px] border border-white/20 rounded px-1.5">⌘</span>
            <span className="text-[10px] border border-white/20 rounded px-1.5">K</span>
          </div>
        </button>
        <NavItem to="/app/inbox" icon={<Inbox className="h-4 w-4" />} label="Inbox" badge="3" />
        <NavItem to="/app/recent" icon={<Clock className="h-4 w-4" />} label="Recent" />
      </div>

      {/* Scrollable sections */}
      <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-8">
        <SidebarSection title="OVERVIEW" defaultExpanded>
          <NavItem to="/app" icon={<LayoutDashboard className="h-4 w-4" />} label="Dashboard" end />
          <NavItem to="/app/analytics" icon={<BarChart2 className="h-4 w-4" />} label="Analytics" />
          <NavItem to="/app/calendar" icon={<CalendarIcon className="h-4 w-4" />} label="Calendar" />
        </SidebarSection>
        ...
        <SidebarSection 
          ...
          defaultExpanded
        >
          <NavItem to="/app/projects" icon={<FolderOpen className="h-4 w-4" />} label="All Projects" />
          <NavItem to="/app/drafts" icon={<FileText className="h-4 w-4" />} label="Drafts" />
          <NavItem to="/app/published" icon={<BookOpen className="h-4 w-4" />} label="Published" />
        </SidebarSection>

        <SidebarSection title="KNOWLEDGE BASE" defaultExpanded>
          <NavItem to="/app/characters" icon={<Users className="h-4 w-4" />} label="Characters" />
          <NavItem to="/app/graph" icon={<Network className="h-4 w-4" />} label="Knowledge Graph" />
        </SidebarSection>
      </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-sm text-white leading-none font-medium mb-1">Alex M.</span>
      ...
      <CreateProjectModal open={isProjectModalOpen} onOpenChange={setIsProjectModalOpen} />
    </div>
  );
}
  ...
  return (
    ...
    >
      <span className="flex items-center gap-3 min-w-0">
        {icon && <span className="shrink-0 opacity-60 group-hover:opacity-100 transition-opacity">{icon}</span>}
        {dot && (
