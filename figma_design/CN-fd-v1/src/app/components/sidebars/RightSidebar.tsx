    ...
    <div className="flex flex-col h-full bg-[#050505] w-full border-l border-border/40 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 h-16 border-b border-border/40 shrink-0">
        <div className="flex items-center gap-6">
          <button
            ...
            className={cn(
              ...
              activeTab === "ASSISTANT" ? "border-white text-white" : "border-transparent text-muted-foreground hover:text-zinc-300"
            )}
          >
            ASSISTANT
          </button>
          <button
            ...
            className={cn(
              "text-[11px] font-medium tracking-[0.2em] transition-all h-16 border-b-[2px]",
              activeTab === "INFO" ? "border-white text-white" : "border-transparent text-muted-foreground hover:text-zinc-300"
            )}
          >
            INFO
          </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onToggleCollapse} className="text-muted-foreground hover:text-white transition-colors p-1.5 rounded-md hover:bg-white/[0.02]">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        ...
        <div className="flex justify-end">
          <div className="bg-[#111111] border border-border/40 text-[13px] text-zinc-200 p-4 rounded-xl max-w-[85%] leading-relaxed shadow-sm">
            Create a Python script to sort a list of dictionaries by a specific key.
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="rounded-xl border border-border/40 bg-[#0a0a0a] overflow-hidden mt-2 shadow-sm">
            <div className="flex items-center justify-between px-4 py-3 border-b border-border/40 bg-[#050505]">
              <span className="text-[11px] font-mono tracking-wider text-zinc-500">PYTHON</span>
              <div className="flex items-center gap-3">
            ...
            <pre className="p-4 text-[12px] font-mono text-zinc-300 overflow-x-auto leading-[1.6]">
              <span className="text-zinc-500">data</span> = [
                &#123;<span className="text-[#22c55e]">'name'</span>: <span className="text-[#22c55e]">'Alice'</span>, <span className="text-[#22c55e]">'age'</span>: <span className="text-[#f59e0b]">25</span>&#125;,
                &#123;<span className="text-[#22c55e]">'name'</span>: <span className="text-[#22c55e]">'Bob'</span>, <span className="text-[#22c55e]">'age'</span>: <span className="text-[#f59e0b]">20</span>&#125;
              ]

              <span className="text-zinc-500">sorted_data</span> = <span className="text-[#3b82f6]">sorted</span>(data, key=<span className="text-zinc-400">lambda</span> x: x[<span className="text-[#22c55e]">'age'</span>])
      ...
      <div className="p-6 border-t border-border/40 bg-[#050505] shrink-0">
        ...
        <div className="flex items-center gap-2 mb-3">
          <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-md bg-[#0a0a0a] border border-border/50 text-[11px] font-medium text-zinc-400 shadow-sm">
            ...
            <button className="hover:text-white ml-1 transition-colors">×</button>
          </div>
        </div>
...
function InfoTab() {
  return (
    <div className="flex flex-col h-full overflow-y-auto p-8 space-y-12">
      {/* Overview */}
      <section>
        <h3 className="text-[11px] font-medium tracking-[0.2em] text-zinc-500 mb-6 uppercase">Overview</h3>
        <div className="grid grid-cols-2 gap-6 mb-8">
          <div className="bg-[#0a0a0a] p-4 rounded-xl border border-border/40 shadow-sm">
            <div className="text-3xl font-light tracking-tight text-white">12k</div>
            <div className="text-[11px] font-medium tracking-wide text-zinc-500 mt-2 uppercase">Words</div>
          </div>
          <div className="bg-[#0a0a0a] p-4 rounded-xl border border-border/40 shadow-sm">
            <div className="text-3xl font-light tracking-tight text-white">45m</div>
            <div className="text-[11px] font-medium tracking-wide text-zinc-500 mt-2 uppercase">Read Time</div>
          </div>
        </div>
      ...
      {/* Tags */}
      <section>
        <h3 className="text-[11px] font-medium tracking-[0.2em] text-zinc-500 mb-4 uppercase">Tags</h3>
        <div className="flex flex-wrap gap-2.5">
          {["FICTION", "SCI-FI", "NOVEL", "DRAFT"].map(tag => (
            <span key={tag} className="px-3 py-1.5 rounded-[4px] border border-border/50 text-[10px] font-medium tracking-wider text-zinc-400 bg-[#0a0a0a] hover:bg-white/[0.02] hover:text-white transition-colors cursor-pointer shadow-sm">
              {tag}
            </span>
          ))}
        </div>
      ...
      {/* Version History */}
      <section>
        <h3 className="text-[11px] font-medium tracking-[0.2em] text-zinc-500 mb-6 uppercase">Version History</h3>
        <div className="space-y-6 relative pl-4 before:absolute before:left-[3px] before:top-2 before:bottom-2 before:w-[2px] before:bg-border/30">
          {[
            { name: "Chapter 3 Revisions", date: "10:42 AM", desc: "Adjusted dialogue pacing in scene 4." },
            { name: "Character Update", date: "Jan 29", desc: 'Renamed "Kael" to "Kaelen".' },
            { name: "First Draft Complete", date: "Jan 25", desc: "Initial completion of core arc." },
          ].map((v, i) => (
            <div key={i} className="relative">
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-zinc-500 tracking-wide">{v.date}</span>
                </div>
                <p className="text-[12px] text-zinc-500 leading-relaxed mt-0.5">{v.desc}</p>
              </div>
            </div>
          ))}
        </div>
        <button className="text-[12px] font-medium text-zinc-400 hover:text-white mt-6 flex items-center gap-1.5 transition-colors group">
          View full history <span className="transform group-hover:translate-x-1 transition-transform">&rarr;</span>
        </button>
      </section>

      {/* Related Characters */}
      <section>
        <h3 className="text-[11px] font-medium tracking-[0.2em] text-zinc-500 mb-6 uppercase">Related Characters</h3>
        <div className="space-y-4">
          {[
            { name: "Elara Vance", role: "Protagonist", initial: "E", color: "bg-[#3b82f6]" },
            { name: "Jax Kaelen", role: "Antagonist", initial: "J", color: "bg-[#ef4444]" },
            { name: "Sarah Chen", role: "Support", initial: "S", color: "bg-[#22c55e]" },
          ].map(c => (
            <div key={c.name} className="flex items-center justify-between group cursor-pointer p-2 -mx-2 rounded-lg hover:bg-white/[0.02] transition-colors">
              <div className="flex items-center gap-4">
                <Avatar className="w-8 h-8 border border-border/50 shadow-sm">
                  <AvatarFallback className={cn("text-[11px] font-medium text-white", c.color)}>{c.initial}</AvatarFallback>
                </Avatar>
                <span className="text-[13px] font-medium text-zinc-300 group-hover:text-white transition-colors">{c.name}</span>
              </div>
              <span className="text-[11px] font-medium tracking-wide text-zinc-500">{c.role}</span>
            </div>
          ))}
        </div>
      </section>
