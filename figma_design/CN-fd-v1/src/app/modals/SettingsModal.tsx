          ...
          {activeTab !== "general" && (
            <div className="px-8 pt-8 pb-4 border-b border-white/5 flex items-end justify-between shrink-0">
              ...
              <div className="relative">
                <span className="text-[10px] text-gray-500 font-medium tracking-wider mb-1.5 block uppercase">
                  CURRENT LAYER
                </span>
                <button
            ...
            <div className="max-w-2xl mx-auto pb-12">
              ...
              {activeTab === "knowledge" && (
                <div className="space-y-8 animate-in fade-in duration-200">
                  <LayerNotice
                    layer={activeLayer}
                    desc="Lower layers override higher layers. Knowledge base dictates facts and context for AI."
                  />
                  ...
                  <SettingsSection title="Context Data">
                    ...
                    {activeLayer === "project" && (
                      <SettingRow
                        title="Project World-building"
                        description="Specific setting, lore, and history for this project."
                      />
                    )}
                    {activeLayer === "document" && (
                      <SettingRow
                        title="Chapter-specific Facts"
                        description="Details relevant only to this active document."
                      />
                    )}
                  </SettingsSection>
                </div>
                    ...
                    {activeLayer === "project" && (
                      <>
                        <SettingRow
                          ...
                          description="Track character arc changes automatically."
                          defaultChecked
                          inherited
                        />
                        <SettingRow
                          title="Plot consistency enforcement"
                          description="Warn if new text contradicts previous chapters."
                        />
                      </>
                    )}
                    {activeLayer === "document" && (
                      <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                        ...
                        <p className="text-xs text-blue-300/70 mb-3">
                          Memory at the document level is volatile. You can
                          promote important facts to the project layer manually
                          via the AI chat.
                        </p>
                      </div>
                    )}
                  ...
                  {activeLayer === "document" ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                      ...
                      <p className="text-sm text-gray-400 max-w-sm">
                        Personalization is handled at the User or Project level.
                        For document-specific tone, instruct the AI directly via
                        chat.
                      </p>
                    </div>
                  ) : (
                    <>
                      <LayerNotice
                        layer={activeLayer}
                        desc="Define how the AI interacts with you and its writing style."
                      />
                      <SettingsSection title="Tone & Style">
                        ...
                        <SettingRow
                          title="Tone"
                          description="How the AI formats its suggestions."
                          value={
                            activeLayer === "user"
                              ? "Professional"
                              : "Hard Sci-fi (Overridden)"
                          }
                        />
                        {activeLayer === "project" && (
                          <SettingRow
                            title="Inherit User Defaults"
                            description="Revert tone settings to global preferences."
              ...
              {activeTab === "skills" && (
                <div className="space-y-8 animate-in fade-in duration-200">
                  <LayerNotice
                    layer={activeLayer}
                    desc="Skills are enabled by default. Disable them at lower layers to restrict AI capabilities."
                  />

                  <SettingsSection title="Available Skills">
                    <SettingRow
                      title="Web Browsing"
                      description="Allow AI to search the internet for facts."
                      defaultChecked
                    />
                    <SettingRow
                      title="Code Interpreter"
                      description="Allow AI to run Python for data analysis."
                      defaultChecked
                    />
                    <SettingRow
                      title="Image Generation"
                      description="Allow AI to generate concept art."
                      defaultChecked={activeLayer !== "document"}
                      inherited={activeLayer === "document"}
                    />
                    <SettingRow
                      title="Deep Critic"
...
function LayerNotice({ layer, desc }: { layer: string; desc: string }) {
  return (
    <div className="px-4 py-3 rounded-lg border border-white/5 bg-[#111111] flex items-start gap-3 mb-6">
      ...
      <div>
        <span className="text-sm font-medium text-white mb-0.5 block capitalize">
    ...
    <div className="flex items-center justify-between gap-8 p-3 rounded-lg hover:bg-white/[0.02] transition-colors border border-transparent hover:border-white/5">
      <div>
        <div className="flex items-center gap-2 mb-0.5">
          ...
          {inherited && (
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-white/10 text-gray-400 font-medium">
              Inherited
            </span>
          )}
        </div>
      ...
      {value ? (
        <span className="text-sm text-white font-medium bg-[#111111] px-3 py-1.5 rounded-md border border-white/10">
          {value}
        </span>
      ) : (
