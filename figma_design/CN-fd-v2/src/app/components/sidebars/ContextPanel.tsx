...
export function ContextPanel({ onToggleCollapse }: ContextPanelProps) {
  ...
  const [viewMode, setViewMode] = useState<"file-tree" | "outline">("file-tree");
  ...
  return (
    <div className="flex flex-col h-full bg-[#1A1A1A] text-[#F0F0F0] w-full overflow-hidden">
      ...
      <div className="h-[32px] px-[16px] py-[12px] flex items-center justify-between shrink-0 box-content">
          ...
          <DropdownMenu.Portal>
            ...
            >
              ...
              >
                ...
                <span className="text-[14px] font-normal text-[#F0F0F0]">文件树</span>
              </DropdownMenu.Item>
              <DropdownMenu.Item 
                disabled={!isEditor}
              ...
              >
                ...
                <span className="text-[14px] font-normal text-[#F0F0F0]">OUTLINE</span>
              </DropdownMenu.Item>
            </DropdownMenu.Content>
          </DropdownMenu.Portal>
        </DropdownMenu.Root>
        ...
        {viewMode === "file-tree" && (
          <button onClick={() => navigate("/app/editor/new")} className="p-1 text-[#888888] hover:text-[#F0F0F0] hover:bg-[#1E1E1E] rounded-[4px] transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-[16px] pb-[12px] pt-[8px] space-y-4">
        {viewMode === "file-tree" ? (
          <div className="flex flex-col gap-1">
            {/* File Tree Nodes */}
            <FileTreeNode title="CreoNow" icon={<Folder className="w-[16px] h-[16px]" />} isProject defaultExpanded>
              <FileTreeNode title="Drafts" indent={1} defaultExpanded>
                <FileTreeNode title="Chapter 1: The Beginning" indent={2} isDocument isActive={location.pathname.includes('draft-1')} />
                <FileTreeNode title="Chapter 2: Silence" indent={2} isDocument />
              </FileTreeNode>
              <FileTreeNode title="Characters" indent={1}>
                <FileTreeNode title="Protagonist Profile" indent={2} isDocument />
              </FileTreeNode>
            </FileTreeNode>
          </div>
        ) : (
          <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-col items-center justify-center h-[200px] opacity-100 animate-in fade-in duration-150">
              ...
              <div className="text-[14px] font-normal text-[#888888] mb-[4px]">还没有大纲</div>
              <div className="text-[12px] font-normal text-[#555555]">添加标题后自动生成</div>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="flex flex-col w-full">
      <div 
        className={cn(
          ...
          isActive ? "bg-[rgba(240,240,240,0.08)] text-[#F0F0F0]" : "hover:bg-[#1E1E1E] text-[#CCCCCC]"
        )}
      ...
      >
        <div className="flex items-center gap-[6px] min-w-0 flex-1">
          ...
          <span className={cn(
            "truncate",
            isProject ? "text-[14px] font-medium text-[#F0F0F0]" : "text-[14px] font-normal"
          )}>
            {title}
          </span>
        </div>
