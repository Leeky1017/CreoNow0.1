...
const tabs: { id: SettingsTab; label: string; icon: React.ReactNode }[] = [
  { id: "general", label: "General Settings", icon: <Layers className="w-[16px] h-[16px]" /> },
  { id: "knowledge", label: "Knowledge Base", icon: <Database className="w-[16px] h-[16px]" /> },
  { id: "memory", label: "Memory Management", icon: <BrainCircuit className="w-[16px] h-[16px]" /> },
  { id: "personalization", label: "Personalization", icon: <Sparkles className="w-[16px] h-[16px]" /> },
  { id: "skills", label: "Skill System", icon: <Cpu className="w-[16px] h-[16px]" /> },
];
...
export function SettingsModal({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <div
      ...
      onClick={(e) => { if (e.target === e.currentTarget) { onOpenChange(false); setLayerDropdownOpen(false); } }}
    >
      <div className="flex w-full max-w-4xl h-[75vh] bg-[#1A1A1A] border border-[#2A2A2A] rounded-[12px] overflow-hidden shadow-[0_4px_16px_rgba(0,0,0,0.4)]">
        ...
        <div className="flex-1 flex flex-col relative bg-[#0D0D0D]">
          ...
          {activeTab !== "general" && (
            <div className="px-[24px] pt-[24px] pb-[16px] border-b border-[#2A2A2A] flex items-end justify-between shrink-0">
              ...
              <div className="relative">
                <span className="text-[12px] text-[#888888] font-medium tracking-[0.05em] mb-[4px] block uppercase">CURRENT LAYER</span>
                <button
          ...
          {/* Dynamic Content */}
          <div className="flex-1 overflow-y-auto p-[24px]" onClick={() => setLayerDropdownOpen(false)}>
            <div className="max-w-2xl mx-auto pb-[48px]">
              {activeTab === "general" && <GeneralSettings />}
              {activeTab === "knowledge" && (
                <div className="space-y-[24px]">
                  <LayerNotice layer={activeLayer} desc="Lower layers override higher layers. Knowledge base dictates facts and context for Agent." />
                  <SettingsSection title="Context Data">
                    <SettingRow title="Global Lore & References" description="Cross-project references automatically applied." defaultChecked />
                  </SettingsSection>
                </div>
              )}
              {activeTab === "memory" && (
              ...
              {activeTab === "personalization" && (
                <div className="space-y-[24px]">
                  <LayerNotice layer={activeLayer} desc="Define how the Agent interacts with you and its writing style." />
                  <SettingsSection title="Tone & Style">
                    <SettingRow title="Language" description="Default language for Agent responses." value="English" />
                    <SettingRow title="Tone" description="How the Agent formats its suggestions." value="Professional" />
                  </SettingsSection>
                </div>
              )}
              {activeTab === "skills" && (
                <div className="space-y-[24px]">
                  <LayerNotice layer={activeLayer} desc="Skills are enabled by default. Disable them at lower layers to restrict Agent capabilities." />
                  <SettingsSection title="Available Skills">
                    <SettingRow title="Web Browsing" description="Allow Agent to search the internet for facts." defaultChecked />
                    <SettingRow title="Code Interpreter" description="Allow Agent to run Python for data analysis." defaultChecked />
...
function GeneralSettings() {
  return (
    <div className="space-y-[24px] mt-[24px]">
      <div>
        <h1 className="text-[28px] font-bold text-[#F0F0F0] leading-[1.2] tracking-[-0.01em] mb-[8px]">General Options</h1>
        <p className="text-[14px] text-[#888888] leading-[1.6]">Customize your writing environment and workflow preferences.</p>
      </div>
...
function LayerNotice({ layer, desc }: { layer: string; desc: string }) {
  return (
    <div className="px-[16px] py-[12px] rounded-[8px] border border-[#2A2A2A] bg-[#111111] flex items-start gap-[12px]">
      ...
      <div>
        <span className="text-[14px] font-medium text-[#F0F0F0] mb-[4px] block capitalize">Currently editing: {layer} Layer</span>
...
function SettingsSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section>
      <h3 className="text-[12px] font-medium tracking-[0.05em] text-[#888888] mb-[12px] pb-[8px] border-b border-[#2A2A2A] uppercase">{title}</h3>
      <div className="space-y-[8px]">{children}</div>
    </section>
  );
}

function SettingRow({ title, description, defaultChecked, value }: { title: string; description: string; defaultChecked?: boolean; value?: string }) {
  return (
    <div className="flex items-center justify-between gap-[24px] p-[12px] rounded-[8px] hover:bg-[#1E1E1E] transition-colors duration-150">
      <div>
        <h4 className="text-[14px] font-medium text-[#F0F0F0] mb-[4px]">{title}</h4>
        <p className="text-[12px] text-[#888888] leading-[1.4]">{description}</p>
      </div>
      {value ? (
        <span className="text-[14px] text-[#F0F0F0] font-medium bg-[#111111] px-[12px] py-[4px] rounded-[6px] border border-[#2A2A2A]">{value}</span>
      ) : (
