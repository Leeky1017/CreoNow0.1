...
import { Plus, Search, User, Bot, AlertTriangle, BarChart2, Lightbulb, X, Check, Pencil } from "lucide-react";
...
const initialCharacters: Character[] = [
  {
    id: 1, name: "Elara Vance", role: "主角", group: "主要角色", age: "24", archetype: "The Reluctant Hero",
    ...
    agentInsights: [
      { type: "warn", text: "潜在不一致：Elara 的眼睛颜色在第 3 章（碧绿）和第 7 章（深蓝）的描述不同" },
      { type: "stat", text: "Elara 在 23 个场景中出现，主要与 Marcus 互动（15 次）" },
      { type: "suggest", text: "人物弧光评估：目前缺少明确的内在冲突转折点，建议在第 15-18 章加入" },
    ],
  },
  {
    id: 2, name: "Kaelen Thorne", role: "反派", group: "主要角色", age: "32", archetype: "The Fallen Paragon",
    description: "Former commander of the Vanguard fleet. Now operates independently, hunting down the same coordinates as Elara.",
    traits: ["Calculating", "Ruthless", "Charismatic"],
    relations: [{ name: "Elara Vance", type: "Target", id: 1 }],
    appearances: [{ chapter: "Chapter 2: The Ambush", count: 6, time: "3d ago" }],
    agentInsights: [{ type: "suggest", text: "角色动机需要更多层次，建议加入同情面" }],
  },
  {
    id: 3, name: "Darius", role: "伙伴", group: "主要角色", age: "28", archetype: "The Loyal Companion",
    description: "An engineer who can fix any ship engine with duct tape and willpower.",
    traits: ["Resourceful", "Anxious", "Brilliant"],
    relations: [{ name: "Elara Vance", type: "Ally", id: 1 }],
    appearances: [{ chapter: "Chapter 1: The Awakening", count: 9, time: "2d ago" }],
    agentInsights: [],
  },
  {
    id: 4, name: "Jax", role: "导师", group: "次要角色", age: "55", archetype: "The Wise Sage",
    ...
    appearances: [{ chapter: "Chapter 3: Void Drift", count: 4, time: "5d ago" }],
    agentInsights: [],
  },
  {
    id: 5, name: "Sarah", role: "信息贩子", group: "次要角色", age: "22", archetype: "The Information Broker",
            ...
            return (
              <div key={group} className="mb-[4px]">
                <div className="sticky top-0 z-[10] bg-[#141414] flex items-center justify-between h-[32px] px-[16px]">
                  <span className="text-[11px] font-medium text-[#555555] uppercase tracking-[0.06em]">{group}</span>
                ...
                {groupChars.map((c, i) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-[13px] font-medium text-[#CCCCCC] truncate">{c.name}</div>
                      <div className="text-[11px] text-[#555555] mt-[2px]">{c.role}</div>
                    </div>
                  </motion.div>
                ))}
              </div>
...
function EditableField({ value, onChange, label, multiline = false }: {
  value: string;
  ...
  if (editing) {
    return (
      <div className="flex items-start gap-[8px]">
        ...
        <button onClick={save} className="p-[4px] text-[#4ADE80] hover:text-[#6EE7A0] transition-colors">
          <Check className="w-[14px] h-[14px]" />
        </button>
        <button onClick={cancel} className="p-[4px] text-[#888888] hover:text-[#F0F0F0] transition-colors">
  ...
  return (
    <div className="group flex items-start gap-[6px] cursor-pointer" onClick={() => setEditing(true)}>
      {multiline ? (
        <p className="text-[14px] text-[#CCCCCC] leading-[1.6] flex-1">{value || <span className="text-[#555555] italic">Click to edit…</span>}</p>
      ) : (
        <span className="text-[14px] text-[#F0F0F0]">{value || <span className="text-[#555555] italic">—</span>}</span>
      )}
...
/* ────── Editable Traits ────── */
function EditableTraits({ traits, onChange }: { traits: string[]; onChange: (traits: string[]) => void }) {
  ...
  return (
    <div className="flex flex-wrap gap-[8px] items-center">
      {traits.map((t, i) => (
        ...
        >
          {t}
          <button onClick={() => removeTrait(i)} className="opacity-0 group-hover:opacity-100 transition-opacity text-[#888888] hover:text-[#F87171]">
        ...
        <div className="flex items-center gap-[4px]">
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "Escape") { setAdding(false); setNewTrait(""); }
            }}
...
/* ────── Character Detail ────── */
function CharacterDetail({ character, onUpdate, onSelectCharacter }: {
  character: Character;
  ...
  return (
    ...
    >
      ...
      <div className="mt-[28px] grid grid-cols-2 gap-x-[24px] gap-y-[20px]">
        <FieldBlock label="年龄">
          <EditableField label="Age" value={character.age} onChange={(age) => onUpdate({ age })} />
        </FieldBlock>
        <FieldBlock label="原型">
          <EditableField label="Archetype" value={character.archetype} onChange={(archetype) => onUpdate({ archetype })} />
        </FieldBlock>
        <FieldBlock label="阵营">
          <EditableField label="Role" value={character.role} onChange={(role) => onUpdate({ role })} />
        </FieldBlock>
        <FieldBlock label="首次出场">
          <span className="text-[14px] text-[#F0F0F0]">{character.appearances[0]?.chapter || "—"}</span>
        </FieldBlock>
      </div>

      {/* Description */}
      <div className="mt-[24px]">
        <span className="text-[11px] text-[#555555] leading-[1.4] uppercase tracking-[0.06em]">外貌与描述</span>
        <div className="mt-[8px]">
          <EditableField
            label="Description"
      ...
      {/* Traits */}
      <div className="mt-[24px]">
        <span className="text-[11px] text-[#555555] leading-[1.4] uppercase tracking-[0.06em]">性格特征</span>
      ...
      <div className="mt-[36px]">
        <div className="flex items-center justify-between mb-[12px]">
          <h3 className="text-[14px] font-semibold text-[#F0F0F0] leading-[1.4] uppercase tracking-[0.01em]">关系网络</h3>
            ...
            {character.relations.map((r) => (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[13px] font-medium text-[#CCCCCC] mt-[2px] truncate">{r.name}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mt-[36px]">
        <div className="flex items-center justify-between mb-[12px]">
          <h3 className="text-[14px] font-semibold text-[#F0F0F0] leading-[1.4] uppercase tracking-[0.01em]">出场统计</h3>
          <span className="min-w-[18px] h-[16px] bg-[#1E1E1E] rounded-full flex items-center justify-center text-[10px] font-medium text-[#555555] px-[4px]">
            ...
            {character.appearances.map((a, i) => (
              <div key={a.chapter}>
                <div className="flex items-center h-[40px] hover:bg-[rgba(255,255,255,0.02)] rounded-[4px] px-[8px] -mx-[8px] transition-colors duration-100 cursor-pointer">
                  <span className="flex-1 text-[13px] text-[#CCCCCC]">{a.chapter}</span>
                  <span className="text-[12px] text-[#555555] mr-[16px] tabular-nums">{a.count} 次</span>
                  <span className="text-[12px] text-[#555555]">{a.time}</span>
                </div>
                {i < character.appearances.length - 1 && <div className="h-[1px] bg-[#1E1E1E]" />}
              </div>
            ))}
          </div>
        ) : (
      ...
      {character.agentInsights.length > 0 && (
        <div className="mt-[36px]">
          <div className="flex items-center gap-[8px] mb-[12px]">
            ...
            <h3 className="text-[14px] font-semibold text-[#F0F0F0] leading-[1.4] uppercase tracking-[0.01em]">Creo 分析</h3>
          </div>
          <div className="flex flex-col gap-[12px]">
            {character.agentInsights.map((insight, i) => (
              ...
              >
                {insight.type === "warn" && <AlertTriangle className="w-[14px] h-[14px] text-[#F87171] shrink-0 mt-[2px]" />}
                {insight.type === "stat" && <BarChart2 className="w-[14px] h-[14px] text-[#555555] shrink-0 mt-[2px]" />}
                {insight.type === "suggest" && <Lightbulb className="w-[14px] h-[14px] text-[#7AA2F7] shrink-0 mt-[2px]" />}
                <span className="text-[13px] text-[#888888] leading-[1.65] line-clamp-2">{insight.text}</span>
              </div>
            ))}
          </div>
...
function FieldBlock({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <span className="text-[11px] text-[#555555] leading-[1.4] uppercase tracking-[0.06em]">{label}</span>
