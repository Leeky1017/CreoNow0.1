...
const NODES = [
  {
    ...
    tags: ["Age: 24", "Human"],
    desc: "A skilled weaver of arcana who seeks to unravel the mystery of the Silent Void.",
  },
  {
    id: "key",
    title: "Crystal Key",
    ...
    desc: "An ancient artifact glowing with an inner cyan light.",
  },
  {
    id: "keep",
    title: "Shadow Keep",
    ...
    desc: "A forbidden fortress located in the center of the Void.",
  },
  {
    id: "war",
    title: "The Great War",
    ...
    shape: "hex" as const,
    tags: ["100 years ago"],
    desc: "A devastating conflict that shattered the old empire.",
  },
];
...
const nodeIcon = (type: string, size: number) => {
  ...
  switch (type) {
    case "角色":
      return <User style={{ width: size, height: size, color }} />;
    case "地点":
      return <MapPin style={{ width: size, height: size, color }} />;
    case "事件":
      return <Clock style={{ width: size, height: size, color }} />;
    case "物品":
      return <Key style={{ width: size, height: size, color }} />;
    default:
      return null;
  }
};
...
export function KnowledgeGraphPage() {
  ...
  return (
    <div
      className="flex flex-col h-full bg-[#0D0D0D] text-[#CCCCCC] relative overflow-hidden"
      onClick={() => setSelected(null)}
    >
      {/* Toolbar */}
      <header
        className="flex items-center justify-between px-[16px] h-[48px] bg-[#0D0D0D] border-b border-[#1E1E1E] shrink-0 z-[20]"
      ...
      >
          ...
          {["角色", "地点", "事件", "物品"].map((t) => (
            ...
            >
              <span
                ...
                style={{
                  backgroundColor: filters.has(t) ? typeColors[t] : "#2A2A2A",
                }}
              />
              {t}
            </button>
        ...
        <div className="flex items-center w-[220px] h-[32px] bg-[#111111] border border-[#1E1E1E] rounded-[6px] px-[8px] gap-[6px] focus-within:border-[#2A2A2A] transition-colors duration-200">
          ...
          <input
            placeholder="搜索实体…"
            className="flex-1 bg-transparent text-[12px] text-[#F0F0F0] placeholder:text-[#3A3A3A] outline-none"
          />
        </div>
      ...
      <div className="flex-1 relative cursor-grab active:cursor-grabbing">
        ...
        {/* Controls */}
        <div
          className="absolute bottom-[16px] right-[16px] flex flex-col gap-[8px] z-[15]"
          onClick={(e) => e.stopPropagation()}
        >
        ...
        {activeNode && (
          ...
          >
            <div className="p-[20px] border-b border-[#1E1E1E] flex items-start justify-between shrink-0">
              <div>
                <h3 className="text-[24px] font-bold text-[#F0F0F0] leading-[1.2] tracking-[-0.02em]">
                  {activeNode.title}
                </h3>
                <div className="flex items-center gap-[8px] mt-[8px]">
                  <span
                    className="inline-block w-[6px] h-[6px] rounded-full"
                    style={{ backgroundColor: typeColors[activeNode.type] }}
                  />
                  <span className="text-[11px] text-[#555555] tracking-[0.06em] uppercase">
                    {activeNode.subtitle}
                  </span>
              ...
              <button
                onClick={() => setSelected(null)}
                className="text-[#555555] hover:text-[#888888] transition-colors duration-150 p-[4px] rounded-[4px] hover:bg-[#1E1E1E]"
              >
            ...
            <div className="flex-1 overflow-y-auto p-[20px] space-y-[24px]">
              <div className="flex flex-wrap gap-[6px]">
                {activeNode.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-[8px] py-[3px] bg-[#141414] border border-[#1E1E1E] rounded-[4px] text-[11px] text-[#888888]"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <div>
                <h4 className="text-[11px] text-[#555555] mb-[8px] tracking-[0.06em] uppercase">
                  Description
                </h4>
                <p className="text-[13px] text-[#888888] leading-[1.65]">
                  {activeNode.desc}
                </p>
              </div>

              <div>
                <h4 className="text-[11px] text-[#555555] mb-[10px] tracking-[0.06em] uppercase">
                  Connected Nodes
                </h4>
                <div className="space-y-[8px]">
                  {EDGES.filter(
                    ...
                    const otherId =
                      edge.from === activeNode.id ? edge.to : edge.from;
                      ...
                      >
                        <div className="flex items-center gap-[10px]">
                          ...
                          <div>
                            ...
                            <div className="text-[10px] text-[#555555] mt-[2px]">
                              {other.type}
                            </div>
                          </div>
                        </div>
