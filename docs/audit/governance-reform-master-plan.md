...
核心变化：
1. 从 7 条原则精简为 5 条（合并 P3+P5，移除纯文档仪式）
...
详见 [文件清理执行清单](./file-cleanup-execution-list.md)。

| 清理目标 | 数量 | 大小 | 行动 |
|----------|------|------|------|
| `rulebook/tasks/` (active) | 151 dirs | ~3 MB | 全部归档到 `rulebook/archive/`，然后冻结 |
| `rulebook/tasks/archive/` | 114 dirs | ~2.6 MB | 保留不动 |
| `openspec/_ops/task_runs/` | 374 files | ~2.5 MB | 全部移到 `openspec/_ops/archive/task_runs/` |
| `openspec/_ops/reviews/` | ~40 files | ~0.3 MB | 全部移到 `openspec/_ops/archive/reviews/` |
| `openspec/changes/archive/` | 240 dirs | ~6 MB | 保留不动（是历史记录） |
| 治理脚本 | 6 个 | - | 移除不再需要的 |

**不删除任何文件**——全部归档。历史是有价值的，但不应该在活跃路径中制造噪音。
