# CN Backend Code Snapshot (Main Process)

更新时间：2026-02-22 12:52

本文件用于把 CN 后端（Electron 主进程）**代码实况**“喂给你”做审计/知识库构建。内容按你提问的 A–G 分组逐条回答，所有结论尽量附带 **SSOT 文件路径**与**最小关键片段**。

> 路径映射：你提问中的 `src/main/` 在仓库中对应 `apps/desktop/main/src/`。

---

## 🏗️ A. 整体架构与代码组织

### A1. `src/main/`（`apps/desktop/main/src/`）完整文件树（到文件级别）

见附录：

- Appendix A: `apps/desktop/main/src/` file list（`find apps/desktop/main/src -type f | sort`）
...
2. `app.whenReady()`：
   ...
   2. `initDb({ userDataDir, logger })`：打开 SQLite，启用 `foreign_keys` + `WAL`，跑 migrations（失败不阻断启动，但 `db=null`）
   3. `registerIpcHandlers({ db, logger, userDataDir, builtinSkillsDir, env })`：构造服务依赖 + 注册全部 IPC handlers（见 A4）
   4. `createMainWindow(logger)`：创建 BrowserWindow，加载 dev server 或 build 后的 `renderer/index.html`
   5. `before-quit`：尝试 `db.close()`
3. `.catch(logAppInitFatal)`：记录 `app_init_fatal` 并 `app.quit()`
4. `window-all-closed`：非 macOS 直接 quit
...
- 所以 handler 内部不需要重复 try/catch；**即使没抓住异常，也不会把 exception 泄漏到 IPC**，而是被包装成稳定 envelope。
...
### B6. 当前所有表的 CREATE TABLE（或等效 schema）

我用“把 migrations 顺序 apply 到内存 DB，再 dump `sqlite_master.sql`”的方式得到当前 schema（可复现）。完整输出见：

- Appendix B: SQLite schema dump（含 tables/indexes/triggers；跳过了 `vec0` 相关 migration）

补充：`sqlite-vec` 可用时会额外应用 `apps/desktop/main/src/db/migrations/0008_user_memory_vec.sql` 创建 `user_memory_vec`（`vec0`）。
...
- 当前模式更接近“按业务域划分 service，在 service 内直接写 SQL”：
  - `apps/desktop/main/src/services/documents/*`（documents/version/branch/merge）
  - `apps/desktop/main/src/services/kg/*`（KG query/write 拆分相对清晰：`kgQueryService.ts` vs `kgWriteService.ts`）
  - `apps/desktop/main/src/services/memory/*`（memory + episodic repository）
- IPC handlers 负责 transport + 少量参数校验；domain errors 多由 service 返回，再映射成 IPC error（例：`mapDocumentErrorToIpcError`）。

证据：

- KG query/write 分离：`apps/desktop/main/src/services/kg/kgService.ts`
- documents domain error 与 IPC 解耦：`apps/desktop/main/src/services/documents/documentCoreService.ts` + `apps/desktop/main/src/ipc/file.ts`
...
- WAL：是，`conn.pragma("journal_mode = WAL")`（`apps/desktop/main/src/db/init.ts`）
- 外键：是，`conn.pragma("foreign_keys = ON")`
- 连接池：没有；主进程内通常是**单 `better-sqlite3` 连接**共享给 services（`index.ts` 把 `db` 实例传给各 handler/service）
- 并发策略：
  - Electron 主进程 JS 单线程；但 IPC handler 可能并发触发，且 `better-sqlite3` 同步查询会阻塞 event loop
  - 现有工程手段偏向“限流/背压/容量上限”，例如：
    - AI stream push 的 backpressure gate：`apps/desktop/main/src/ipc/pushBackpressure.ts`
    - Context Engine 并发上限：`apps/desktop/main/src/services/context/layerAssemblyService.ts` 的 `CONTEXT_CAPACITY_LIMITS.maxConcurrentByDocument`
...
- 核心装配：`apps/desktop/main/src/services/context/layerAssemblyService.ts`
- 四层类型：`apps/desktop/main/src/services/context/types.ts`（`ContextLayerId = "rules" | "settings" | "retrieved" | "immediate"`）
- Fetchers：
...
- entity/relation 的扩展字段用 `metadata_json`/`attributes_json` 等 JSON 字段承载（具体字段以 schema dump 为准）

查询/遍历（代码层）：

- `apps/desktop/main/src/services/kg/kgQueryService.ts`：subgraph/path/relevant/byIds 等查询接口
- `apps/desktop/main/src/services/kg/entityMatcher.ts`：实体匹配（为 context/rules 注入提供支持）
- `apps/desktop/main/src/services/kg/kgRecognitionRuntime.ts`：识别运行时（配合保存/索引等链路）

### C12. Memory System 三层记忆现状（工作/情景/语义）与蒸馏逻辑？

当前实现更接近“语义记忆 + 情景(episodic) + 注入预览”的组合：

- 基础记忆 CRUD：`apps/desktop/main/src/services/memory/memoryService.ts`（表：`user_memory`）
- 情景/episodic：`apps/desktop/main/src/services/memory/episodicMemoryService.ts`（表：`memory_episodes`）
- 语义规则/占位：`memory_semantic_placeholders`（见 Appendix B）
- “情景 → 语义”的蒸馏入口已存在：
  - IPC：`apps/desktop/main/src/ipc/memory.ts` 的 `"memory:semantic:distill"` handler
  - service：`episodicService.distillSemanticMemory(...)`（具体逻辑见 `episodicMemoryService.ts`）
  - 进度 push：`memory:distill:progress`（main process `webContents.send` 广播）
- 语义 recall 的 vector 索引是 **可选增强**：
  - `apps/desktop/main/src/services/memory/userMemoryVec.ts` + migration `0008_user_memory_vec.sql`
  - `sqlite-vec` 不可用会降级（`initDb()` 的 `tryLoadSqliteVec()`）
...
- 列表见：`apps/desktop/main/skills/packages/pkg.creonow.builtin/1.0.0/skills/`

自定义技能（custom）：

- 表：`custom_skills`（见 Appendix B）
- IPC：`skill:custom:*` / `skill:registry:*`（见 `packages/shared/types/ipc-generated.ts`）

执行 pipeline（真实链路）：

1. Renderer 调 IPC：`ai:skill:run`
2. Main IPC handler：`apps/desktop/main/src/ipc/ai.ts`
...
3. LLM 调用：`apps/desktop/main/src/services/ai/aiService.ts`（fetch + SSE）
4. Streaming：main `webContents.send(SKILL_STREAM_CHUNK_CHANNEL, event)` 推送到 renderer（见 D17）

关键证据：

- scheduler：`apps/desktop/main/src/services/skills/skillScheduler.ts`（`globalConcurrencyLimit` / `sessionQueueLimit`）
- executor：`apps/desktop/main/src/services/skills/skillExecutor.ts`（context assemble + 输出约束校验）

### C14. Version Control 快照存完整文档还是增量 diff？存储格式？

结论（基于 schema + documents service）：

- 快照以 **完整内容** 为主：`documents` 与 `document_versions` 都持久化 `content_json`（TipTap JSON）、`content_text`（派生纯文本）、`content_md`（派生 markdown）。
- 同时存在 diff 辅助字段（用于对比/展示/回滚）：
  - `document_versions.diff_format` / `document_versions.diff_text`（见 migrations/Appendix B）
- 支持 branch/merge/conflict 相关表：`document_branches` / `document_merge_sessions` / `document_merge_conflicts`（见 Appendix B）

证据：

- schema：Appendix B 的 `documents` / `document_versions` / `document_branches` 等
- 领域实现：`apps/desktop/main/src/services/documents/documentCoreService.ts`（生成 snapshot + diff + merge）
...
- 有 provider 抽象（但不是 SDK wrapper，而是“配置解析 + fetch 调用 + SSE 解析”）：
  - provider 解析：`apps/desktop/main/src/services/ai/providerResolver.ts`
  - 运行入口：`apps/desktop/main/src/services/ai/aiService.ts`
- 不是 `import OpenAI` 这种 SDK 直连模式；核心是 `fetch()` + SSE 解析（OpenAI/Anthropic 都走 SSE）。
...
- `openai`
- `anthropic`
- `proxy`（openai-compatible / upstream proxy）

切换方式：

- 通过 env / DB settings（见 `AiProxySettingsService` + `ProviderResolver`）；不需要改业务代码。

证据：

- provider types：`apps/desktop/main/src/services/ai/providerResolver.ts`（`type AiProvider = "anthropic" | "openai" | "proxy"`）
- settings 持久化：`apps/desktop/main/src/services/ai/aiProxySettingsService.ts`

### D17. 流式输出（streaming）实现方式？

链路：

- 后端调用上游时用 SSE（`readSse()`）：`apps/desktop/main/src/services/ai/aiService.ts`
...
1. builtin skills：`apps/desktop/main/skills/**/SKILL.md`（每个技能的模板/约束）
2. AI service 固定 prompt 片段：
   - `apps/desktop/main/src/services/ai/identityPrompt.ts`
   - `apps/desktop/main/src/services/ai/assembleSystemPrompt.ts`
3. custom skills：DB 表 `custom_skills.prompt_template`（见 Appendix B）+ IPC CRUD

### D19. 队列/并发控制/重试/超时？

已实现的工程机制（证据导向）：

- **队列与并发**：`apps/desktop/main/src/services/skills/skillScheduler.ts`
  - global 并发上限（默认 8）
  - session FIFO 队列 + queue overflow guard（默认 20）
- **超时**：
  - IPC 层默认 30s：`apps/desktop/main/src/ipc/runtime-validation.ts`（`createValidatedIpcMain({ defaultTimeoutMs: 30_000 })`）
  - LLM 调用 timeout 来自 provider config：`ProviderConfig.timeoutMs`（`providerResolver.ts`）
- **重试/熔断/half-open**：
  - `aiService.ts` 内有 rate limit、failover、half-open（常量：`PROVIDER_HALF_OPEN_AFTER_MS`）
- **stream push 背压**：`apps/desktop/main/src/ipc/pushBackpressure.ts`（chunk 可丢弃，控制事件必达）
...
- contract：`apps/desktop/main/src/ipc/contract/ipc-contract.ts`
- generated typemap：`packages/shared/types/ipc-generated.ts`

当前通道总数：142（从 `packages/shared/types/ipc-generated.ts` 的 `IPC_CHANNELS` 解析得到）

清单见：

- Appendix C: IPC channel list（逐行列出 142 个 channel）
...
- handler 文件按域拆分：`apps/desktop/main/src/ipc/*.ts`（例如 `ipc/ai.ts`, `ipc/memory.ts`, `ipc/knowledgeGraph.ts`）

### E22. 是否实现 Schema-first Type Map？

是。

- contract SSOT：`apps/desktop/main/src/ipc/contract/ipc-contract.ts`
- 生成物：`packages/shared/types/ipc-generated.ts`（`IpcChannelSpec` + `IpcRequest/C` + `IpcInvokeResult/C` 等）
- runtime 层面也做 schema 校验：`apps/desktop/main/src/ipc/runtime-validation.ts`

### E23. preload 暴露了什么？`window.electronApi` 的类型定义在哪里？
...
- request/response schema 校验：`apps/desktop/main/src/ipc/runtime-validation.ts`
- schema 来源：`apps/desktop/main/src/ipc/contract/schema.ts`

---

## 🔧 F. 工程与质量

### F25. 后端目前有多少测试？单元/集成/E2E 比例？覆盖率？
...
- 有脚本入口：`apps/desktop/package.json` 的 `test:coverage`（`vitest run --coverage`）
- 覆盖率报告默认不在仓库内（需要本地运行生成）

### F26. 哪些模块完全没有测试？

在 `apps/desktop/main/src/services/` 下，“没有 colocated unit tests（`__tests__`）”的目录包括：

- `apps/desktop/main/src/services/judge/`
- `apps/desktop/main/src/services/search/`
- `apps/desktop/main/src/services/stats/`

注意：这不等于“完全无覆盖”，这些域在 `apps/desktop/tests/integration/` 与 `apps/desktop/tests/e2e/` 中有覆盖点（例如 judge / search / stats）。

### F27. 已知技术债 / 反模式（对照历史审计，哪些已修/未修）

本仓库根目录 `Opus审计完整版.md`（历史审计）中，后端相关的几项现在的状态（以当前代码实况核对）：

- **已修**：`@shared/*` path alias（历史问题：相对路径 5-6 层 `../`）
  - 证据：当前主进程普遍使用 `@shared/...`（例如 `apps/desktop/main/src/index.ts`）
- **已修**：AI model 参数硬编码 `"fake"`（历史问题 2.1）
  - 证据：当前 IPC contract `ai:skill:run` request 里有 `model` 字段；`apps/desktop/main/src/ipc/ai.ts` 的 payload 明确要求 `model: string`
- **部分修复/需确认**：Chat history / feedback 的持久化语义
  - 当前已有 `ai:chat:*` / `ai:skill:feedback` 通道与 `skill_feedback` 表；但 chat history 是否跨重启持久化，需要再看 `apps/desktop/main/src/ipc/ai.ts` 的实现细节与是否落库（本文件不做推断，只列事实入口）

如果你希望我把 `Opus审计完整版.md` 的后端条目逐条对照当前代码并标注 `FIXED/NOT_FIXED/PARTIAL`，我可以在下一轮补一份“对照表”。

### F28. 性能瓶颈点？

代码中已经显式做了“性能风险点的护栏”，可视为潜在瓶颈位置：

- 主进程同步 SQLite（`better-sqlite3`）在高频 IPC 下可能阻塞 event loop（B9）
- Context Engine 有 SLO 阈值与并发上限：`apps/desktop/main/src/services/context/layerAssemblyService.ts`（`CONTEXT_SLO_THRESHOLDS_MS`, `maxConcurrentByDocument`）
- AI streaming push 有背压丢弃策略：`apps/desktop/main/src/ipc/pushBackpressure.ts`

### F29. 日志系统？

主进程日志是结构化 JSONL，写入 `userData/logs/main.log`：

- `apps/desktop/main/src/logging/logger.ts`（`createMainLogger()` → `fs.appendFileSync(JSON.stringify(record))`）
...
- `apps/desktop/main/src/index.ts` 把 `safeStorage` 封装成 `secretStorage` 依赖注入给 AI IPC handlers

### G31. 渲染进程输入在主进程侧是否 sanitize/validate？

已实现两层防线：

1. **IPC runtime schema 校验**：所有通道在 `createValidatedIpcMain()` 边界校验 request/response（A4/E24）
2. **业务域级别的额外 normalize/guard**（举例）：
   - 文件系统路径 traversal 防护：`apps/desktop/main/src/services/context/contextFs.ts`（规范化 `.creonow/**` 路径）
   - baseUrl/apiKey 规范化：`apps/desktop/main/src/services/ai/aiProxySettingsService.ts`（`normalizeBaseUrl`, `normalizeApiKey`）
   - project/session 绑定隔离：`apps/desktop/main/src/ipc/projectSessionBinding.ts` + `apps/desktop/main/src/ipc/projectAccessGuard.ts`

### G32. 文件 I/O 是否有原子写入保护？

结论（以扫描到的写入点为准）：

- 多处直接 `writeFile`/`writeFileSync`，**未统一采用 temp+rename 的原子写入模式**。
- 但关键业务数据 SSOT 多在 SQLite 内（事务 + WAL）；文件 I/O 主要集中在 `.creonow/**` 元数据、导出文件、技能文件等。

证据（代表性写入点）：

- `.creonow` 初始化写默认文件：`apps/desktop/main/src/services/context/contextFs.ts`（`fs.writeFileSync` / `fsPromises.writeFile`）
- constraints 落盘：`apps/desktop/main/src/ipc/constraints.ts`（`fs.writeFile`）
- export 写文件：`apps/desktop/main/src/services/export/exportService.ts`（`fs.writeFile`）

---

## Appendix A. `apps/desktop/main/src` File List (209 files)
...
apps/desktop/main/src/vite-raw.d.ts
```

## Appendix B. SQLite Schema Dump (from migrations, vec0 skipped)
...
-- Applied: 21/22
-- Skipped migrations:
--   - 0008_user_memory_vec.sql: uses vec0 (sqlite-vec extension)
...
-- [table] chapter_synopses (table=chapter_synopses)
...
-- [table] custom_skills (table=custom_skills)
...
-- [table] document_branches (table=document_branches)
...
-- [table] document_merge_conflicts (table=document_merge_conflicts)
...
-- [table] document_merge_sessions (table=document_merge_sessions)
...
-- [table] document_versions (table=document_versions)
CREATE TABLE document_versions (
  ...
  created_at INTEGER NOT NULL, reason TEXT NOT NULL DEFAULT '', content_hash TEXT NOT NULL DEFAULT '', diff_format TEXT NOT NULL DEFAULT '', diff_text TEXT NOT NULL DEFAULT '', word_count INTEGER NOT NULL DEFAULT 0,
  FOREIGN KEY(project_id) REFERENCES projects(project_id) ON DELETE CASCADE,
  FOREIGN KEY(document_id) REFERENCES documents(document_id) ON DELETE CASCADE
)
...
CREATE TABLE documents (
  ...
  content_md TEXT NOT NULL,
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL, content_hash TEXT NOT NULL DEFAULT '', type TEXT NOT NULL DEFAULT 'chapter', status TEXT NOT NULL DEFAULT 'draft', sort_order INTEGER NOT NULL DEFAULT 0, parent_id TEXT,
  FOREIGN KEY(project_id) REFERENCES projects(project_id) ON DELETE CASCADE
)

-- [table] documents_fts (table=documents_fts)
...
-- [table] judge_models (table=judge_models)
...
-- [table] kg_entities (table=kg_entities)
CREATE TABLE kg_entities (
  ...
  updated_at TEXT NOT NULL, ai_context_level TEXT NOT NULL DEFAULT 'when_detected', aliases TEXT NOT NULL DEFAULT '[]', last_seen_state TEXT,
  FOREIGN KEY(project_id) REFERENCES projects(project_id) ON DELETE CASCADE
)

-- [table] kg_relation_types (table=kg_relation_types)
...
-- [table] kg_relations (table=kg_relations)
...
-- [table] memory_episodes (table=memory_episodes)
...
-- [table] memory_semantic_placeholders (table=memory_semantic_placeholders)
...
CREATE TABLE projects (
  ...
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL
, archived_at INTEGER, type TEXT NOT NULL DEFAULT 'novel', description TEXT NOT NULL DEFAULT '', stage TEXT NOT NULL DEFAULT 'outline', target_word_count INTEGER, target_chapter_count INTEGER, narrative_person TEXT NOT NULL DEFAULT 'first', language_style TEXT NOT NULL DEFAULT '', target_audience TEXT NOT NULL DEFAULT '', default_skill_set_id TEXT, knowledge_graph_id TEXT)
...
-- [table] skill_feedback (table=skill_feedback)
...
-- [table] stats_daily (table=stats_daily)
...
-- [table] trace_feedback (table=trace_feedback)
...
-- [table] user_memory (table=user_memory)
CREATE TABLE user_memory (
  ...
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL,
  deleted_at INTEGER, document_id TEXT DEFAULT NULL,
  FOREIGN KEY(project_id) REFERENCES projects(project_id) ON DELETE CASCADE
)

-- [trigger] documents_ad_fts (table=documents)
CREATE TRIGGER documents_ad_fts AFTER DELETE ON documents BEGIN
  DELETE FROM documents_fts WHERE rowid = old.rowid;
END

-- [trigger] documents_ai_fts (table=documents)
...
-- [trigger] documents_au_fts (table=documents)
