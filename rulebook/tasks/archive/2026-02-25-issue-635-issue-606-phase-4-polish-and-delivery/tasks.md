...
- [x] 1.2 阅读并确认 change scope：`openspec/changes/archive/issue-606-phase-4-polish-and-delivery/{proposal.md,specs/*,tasks.md}`
...
- [x] 2.1 引用并确认 Scenario->测试映射来源：`openspec/changes/archive/issue-606-phase-4-polish-and-delivery/tasks.md`
...
- [x] 4.3 required checks 对齐并通过（`ci` / `openspec-log-guard` / `merge-serial`）

## 5. Refactor（保持绿灯）

- [x] 5.1 回填重构后回归验证证据并确认无行为漂移
- [x] 5.2 汇总 Main Session Audit 签字输入（由主会话最终签字）
...
- [x] 6.4 PR 创建后回填 ISSUE-635 RUN_LOG 的真实 PR URL（禁止占位符）
- [x] 6.5 完成 required checks 全绿 + auto-merge 合并后的 main 收口证据
