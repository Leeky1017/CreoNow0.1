...
- [ ] 1.1 校验 Issue `#641` OPEN 状态并记录证据（若网络受限则记录 blocker）
...
- [x] 1.4 阅读并确认 change 范围：`openspec/changes/issue-606-phase-4-polish-and-delivery/{proposal.md,tasks.md}`
- [x] 1.5 评估并同步 `openspec/changes/EXECUTION_ORDER.md`（如存在状态漂移）
- [x] 1.6 记录依赖同步检查（Dependency Sync Check）输入与初始结论

## 2. TDD Mapping（先测前提）

- [x] 2.1 引用 Scenario->测试映射来源：`openspec/changes/issue-606-phase-4-polish-and-delivery/tasks.md`
...
- [ ] 3.2 若后续发现 execution-order/门禁漂移，补充对应失败证据
...
- [ ] 4.4 网络恢复后补齐 Issue OPEN 与 PR 链接真值

## 5. Refactor（保持绿灯）

- [ ] 5.1 收敛 #641 文档措辞与 `issue-606-phase-4-polish-and-delivery` change 描述
- [ ] 5.2 汇总 Main Session Audit 签字输入（由主会话签字提交完成）
...
- [ ] 6.4 PR 创建后回填 ISSUE-641 RUN_LOG 的真实 PR URL（禁止占位符）
- [ ] 6.5 required checks 全绿 + auto-merge + main 收口证据
