...
- [ ] 1.1 确认 Issue `#620` 为 OPEN 并将真实链接写入 RUN_LOG
...
- [ ] 2.4 汇总 Red/Green/Refactor 证据片段并持续更新 RUN_LOG

## 3. PR & Gate Closure

- [ ] 3.1 创建 PR（base=`main`，head=`task/620-global-hardening-baseline`，body 含 `Closes #620`）
- [ ] 3.2 开启 auto-merge，跟踪 required checks：`ci` / `openspec-log-guard` / `merge-serial`
- [ ] 3.3 确认 PR 合并、主干同步与 Rulebook 归档收口

## Blockers / Notes

- GitHub API 当前不可达（`gh issue view 620` 返回 `error connecting to api.github.com`），导致 1.1 与 3.x 需等待网络恢复后继续。
