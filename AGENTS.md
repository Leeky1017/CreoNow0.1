...
| 四  | 双体系协作 | OpenSpec + GitHub 的职责与关系             |
...
1. CI 失败和修复过程必须记录在 PR comment 中
2. 遇到阻塞必须记录并通知，不得静默放弃
3. PR 描述必须包含变更摘要和验证证据

✅ CI 失败 → 修复 → push → 在 PR comment 记录失败原因和修复过程

✅ 遇到 blocker → PR comment 记录 → 通知 Owner → 等待
...
1. 两个 required checks：`ci`、`merge-serial`
...
2. PR 合并后才能将 delta spec 归档到主 spec
3. 若 change 存在上游依赖，需先确认上游状态再继续
...
2. **GitHub**：以 Issue、Branch、PR、required checks、auto-merge 作为验收门禁。
...
| 2. 规格制定   | spec 已编写或更新；若有上游依赖则已确认上游状态                                                     |
...
3. 禁止 Feature 层（L3）使用原生 HTML 交互元素（`<button>`、`<input>`、`<select>`）——必须使用 L1 Primitives（审计根因：75 文件 / 357 处违规）
4. 禁止使用 Tailwind 内置阴影类（`shadow-lg`、`shadow-xl`、`shadow-2xl`）——必须走 `--shadow-*` Design Token（审计根因：19 文件违规）
5. 禁止在 JSX 中使用裸字符串字面量——所有用户可见文本必须走 `t()` / i18n（审计根因：~500+ 处硬编码）
